...
```{r}
# txdb <- makeTxDbFromGFF(file = "/lustre/scratch118/malaria/team222/jr35/Pf3D7_genomes_gtfs_indexs/PlasmoDB-52_Pfalciparum3D7.gff", format = "gff") 
...
#     ungroup())%>% 
#     map(., ~ggplot(.x, aes(x=total_unspliced,y=intron_size, color = 'isoforms')) + 
#         geom_point() + 
#         theme(text = element_text(size=20))) %>%
# plot_grid(plotlist = ., labels = scv_bf2_names, ncol = 5)
# # options(repr.plot.width=7, repr.plot.height=7)
```

```{r, fig.width= 32, fig.height=14}
# options(repr.plot.width=32, repr.plot.height=14)

intron_size_vs_spliced <- ef_scv_stochastic %>% 
map(. %>%
    .$var %>%
    dplyr::select('Gene_ID', 'total_unspliced', 'mean_unspliced', 'n_cells_by_unspliced', 'total_spliced', 'mean_spliced', 'n_cells_by_spliced') %>% 
    left_join(., gene_intron_size, by=c('Gene_ID'='gene_id'))) %>%
map(., ~ggscatter(., x='total_unspliced',y='intron_size', color = 'isoforms',
                  add = "reg.line",  # Add regressin line
                  add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line
                  conf.int = TRUE  # Add confidence interval) 
                 )
   ) %>%
map(~. + stat_cor(method = "pearson", label.x = 3000, label.y = 3000))
# stat_cor(method = "pearson", label.x = 50, label.y = 200)

plot_grid(plotlist = intron_size_vs_spliced, labels = scv_names, ncol = 3)
# options(repr.plot.width=7, repr.plot.height=7)
```
...
map(. %>%
    ...
    dplyr::select('Gene_ID', 'total_unspliced', 'mean_unspliced', 'n_cells_by_unspliced', 'total_spliced', 'mean_spliced', 'n_cells_by_spliced') %>% 
    left_join(., gene_intron_size, by=c('Gene_ID'='gene_id'))) %>%
    map(., ~ggplot(.x, aes(x=log1p(total_unspliced),y=log1p(intron_size), color = isoforms)) + 
        geom_point() + 
        theme(text = element_text(size=20))) %>%
plot_grid(plotlist = ., labels = scv_names, ncol = 5)
...
map(. %>%
    ...
    map(., ~ggplot(.x, aes(x=log1p(intron_size),y=log1p(total_unspliced), color = isoforms)) +
        geom_point() + 
        geom_smooth(method='lm', formula= y~x)+ 
        stat_cor(method = "pearson", label.x = 5, label.y = 4.5)+
        theme(text = element_text(size=20))) %>%
plot_grid(plotlist = ., labels = names(ef_scv_stochastic), ncol = 5)
...
#       dplyr::select(c("gene", "method", i)) %>%
#       tidyr::spread(key = "method", value = i) %>% 
...
# list(ef_scv_stochastic,bf2_vels)  %>%
# pmap(~ ..1$var %>%
#     dplyr::select(Gene_ID, velocity_genes, fit_alpha, fit_beta, total_unspliced,anno_300cells) %>%
#     left_join(., colSums(..2) %>% data.frame() %>% dplyr::rename('velo_sums'='.') %>%rownames_to_column('bcode'), by=c('Gene_ID'='bcode') ) %>%
#     mutate(`Velocities estimated` = ifelse(is.na(velo_sums), 'No', 'Yes')) %>%
#      # filter(!is.na(velo_sums)) %>%
#     dplyr::count(anno_300cells)
#     ) %>%
# dplyr::bind_rows(., .id='Days') %>%
# ggplot(., aes(x=Days, y=n, fill=anno_300cells)) +
# geom_col()
```
...
corr_genes %>% 
map(. %>%
    dplyr::filter(qval < 0.00005 & corr > 0.5) %>%
    arrange(desc(corr)) %>%
   pull(`Gene`))
```
...
corr_genes %>% 
map(. %>%
    dplyr::filter(qval < 0.005 & corr > 0.1) %>%
...
corr_genes %>% 
map(. %>%
    dplyr::filter(pval < 0.00005 & corr > 0.4) %>%
    arrange(desc(corr)) %>%
   pull(`Gene`))
```
...
# dr_gns <- corr_genes %>% 
# map(. %>%
#     dplyr::filter(qval < 0.001 & corr > 0.2) %>%
#     group_by(ID) %>%
#     summarise(across('Timepoint', ~(toString(na.omit(.))))) %>%
#     filter(!is.na(ID))
#    ) %>%
# bind_rows(., .id='sex_n_cluster') %>%
# pivot_wider(names_from = 'sex_n_cluster', values_from = 'Timepoint') %>%
# mutate(CDgam_sex = case_when(!is.na(female_drivers_in_CDB) &  is.na(male_drivers_in_CDB) ~ 'Female',
#                              !is.na(male_drivers_in_CDB) & is.na(female_drivers_in_CDB) ~ 'Male',
#                              !is.na(female_drivers_in_CDB) &  !is.na(male_drivers_in_CDB) ~ 'Female,Male'),
#       Bgam_sex = case_when(!is.na(female_drivers_in_gambranch) &  is.na(male_drivers_in_gambranch) ~ 'Female',
#                            !is.na(male_drivers_in_gambranch) & is.na(female_drivers_in_gambranch) ~ 'Male',
#                            !is.na(female_drivers_in_gambranch) &  !is.na(male_drivers_in_gambranch) ~ 'Female,Male'))
```

```{r}
dr_gns_all <- corr_genes %>% 
    map(. %>% select(ID, Timepoint_freq, Gene, source_id, `Gene ID`, qval, corr,Timepoint) %>%
  ...
  bind_rows(., .id='sex_n_cluster') %>%
            pivot_wider(names_from = 'sex_n_cluster', values_from = matches('corr*|qval*|dir*')) %>%
   mutate(CDgam_sex = case_when(if_any(contains('female_drivers_in_CDB'), ~!is.na(.)) &  if_any(contains('male_drivers_in_CDB'), ~is.na(.)) ~ 'Female',
                                if_any(contains('male_drivers_in_CDB'), ~!is.na(.)) &  if_any(contains('female_drivers_in_CDB'), ~is.na(.)) ~ 'Male',
                                if_any(contains('female_drivers_in_CDB'), ~!is.na(.)) &  if_any(contains('male_drivers_in_CDB'), ~!is.na(.)) ~ 'Female,Male'),
          Bgam_sex = case_when(if_any(contains('female_drivers_in_gambranch'), ~!is.na(.)) &  if_any(contains('male_drivers_in_gambranch'), ~is.na(.)) ~ 'Female',
                                if_any(contains('male_drivers_in_gambranch'), ~!is.na(.)) &  if_any(contains('female_drivers_in_gambranch'), ~is.na(.)) ~ 'Male',
                                if_any(contains('female_drivers_in_gambranch'), ~!is.na(.)) &  if_any(contains('male_drivers_in_gambranch'), ~!is.na(.)) ~ 'Female,Male'),
          lR_eT_Asex_gam = case_when(if_any(contains('early_schizont_drivers_in_lReT'), ~!is.na(.)) &  if_any(contains('early_gam_drivers_in_lReT'), ~is.na(.)) ~ 'Asexual',
                                if_any(contains('early_gam_drivers_in_lReT'), ~!is.na(.)) &  if_any(contains('early_schizont_drivers_in_lReT'), ~is.na(.)) ~ 'Gametocyte',
                                if_any(contains('early_schizont_drivers_in_lReT'), ~!is.na(.)) &  if_any(contains('early_gam_drivers_in_lReT'), ~!is.na(.)) ~ 'Asexual,Gametocyte')) %>%
...
```{r}
stocha_dr_gns <- stocha_corr_genes %>% 
map(. %>%
    dplyr::filter(qval < 0.001 & corr > 0.2) %>%
    group_by(ID) %>%
    summarise(across('Timepoint', ~(toString(na.omit(.))))) %>%
...
mutate(CDgam_sex = case_when(!is.na(female_drivers_in_CDB) &  is.na(male_drivers_in_CDB) ~ 'Female',
      ...
      Bgam_sex = case_when(!is.na(female_drivers_in_gambranch) &  is.na(male_drivers_in_gambranch) ~ 'Female',
                           !is.na(male_drivers_in_gambranch) & is.na(female_drivers_in_gambranch) ~ 'Male',
                           !is.na(female_drivers_in_gambranch) &  !is.na(male_drivers_in_gambranch) ~ 'Female,Male'),
      lR_eT_Asex_gam = case_when(!is.na(early_schizont_drivers_in_lReT) &  is.na(early_gam_drivers_in_lReT) ~ 'Asexual',
                                 !is.na(early_gam_drivers_in_lReT) & is.na(early_schizont_drivers_in_lReT) ~ 'Gametocyte',
                                 !is.na(early_schizont_drivers_in_lReT) &  !is.na(early_gam_drivers_in_lReT) ~ 'Asexual,Gametocyte')) 
...
stocha_pk_gns <- stocha_pk_corr_genes %>% 
  ...
  mutate(
    ...
    Asex_gam_stocha_VKCK = case_when(
      if_any(matches('d.*_asexual_drivers_vkck'), ~.== 'correlated') & if_any(matches('d.*_early_gam_drivers_in_lReT_stocha'), ~.== 'correlated') ~ 'Asexual,Gametocyte',
...
```{r}
stocha_dr_gns %>% filter(ID %in% c('PF3D7_0936600'))
```

```{r}
# corr_genes[[4]] %>% 
# # distinct(`Gene ID`, .keep_all = T) %>%
# filter(qval < 0.0005 & (corr > 0.2 | corr < -0.2)) %>%
# dplyr::select(Gene_ID, corr, qval, Timepoint) %>%
# pivot_wider(names_from = Timepoint, values_from = c('corr','qval')) %>%
# tibble::column_to_rownames("ID")
```

```{r}
print(GGally::ggpairs(stocha_corr_genes[[4]] %>%
                          filter(!is.na(ID) & qval < 0.001 ) %>%
                            dplyr::select(ID, corr, Timepoint) %>%
                            pivot_wider(names_from = Timepoint, values_from = c('corr')) %>%
                            tibble::column_to_rownames("ID"),
                          # dplyr::mutate_all(.funs = log1p),
                          lower = list(continuous = GGally::wrap(lowerfun)),
                          title = paste0("Comparison of vo counts between days"), 
                          xlab = paste("log(count + 1)")) +
...
```{r}
stocha_dr_gns_neg <- stocha_corr_genes %>% 
map(. %>%
    dplyr::filter(qval < 0.001 & corr < -0.2) %>%
    group_by(ID) %>%
    summarise(across('Timepoint', ~(toString(na.omit(.))))) %>%
...
mutate(CDgam_sex = case_when(!is.na(female_drivers_in_CDB) &  is.na(male_drivers_in_CDB) ~ 'Female',
      ...
      Bgam_sex = case_when(!is.na(female_drivers_in_gambranch) &  is.na(male_drivers_in_gambranch) ~ 'Female',
                           !is.na(male_drivers_in_gambranch) & is.na(female_drivers_in_gambranch) ~ 'Male',
                           !is.na(female_drivers_in_gambranch) &  !is.na(male_drivers_in_gambranch) ~ 'Female,Male'),
      lR_eT_Asex_gam = case_when(!is.na(early_schizont_drivers_in_lReT) &  is.na(early_gam_drivers_in_lReT) ~ 'Asexual(schizont)',
                                 !is.na(early_gam_drivers_in_lReT) & is.na(early_schizont_drivers_in_lReT) ~ 'Gametocyte',
                                 !is.na(early_schizont_drivers_in_lReT) &  !is.na(early_gam_drivers_in_lReT) ~ 'Asexual(schizont),Gametocyte'))
```

```{r}
stocha_corr_genes[[1]] %>%pull(ID) %>% duplicated() %>% table()
```

```{r, fig.width= 12, fig.height=7}
options(repr.plot.width=12, repr.plot.height=7)

ggvenn(list('all_days' = stocha_dr_gns %>% filter(if_any(contains('drivers'), ~str_detect(.,'all_days'))) %>% pull(ID),
            'all' = stocha_dr_gns %>% filter(if_any(contains('drivers'), ~str_detect(.,'all$'))) %>% pull(ID)#,
            # 'day10a' = stocha_dr_gns %>% filter(if_any(contains('drivers'), ~str_detect(.,'day10'))) %>% pull('Gene ID'),
            # 'day10b' = stocha_dr_gns %>% filter(if_any(contains('drivers'), ~str_detect(.,'day10b'))) %>% pull('Gene ID')
           ),
...
```{r}
##CDB and gambranch correlation and (anti-)correlation
...
corr_clustr_venn(corr_stage1 = 'dir_a.*_female_in_', corr_stage2 = 'dir_a.*_male_in_', sufx='ness')
```

```{r}
##PK male and femaleness correlation and (anti-)correlation
corr_clustr_venn(dset = stocha_pk_gns, corr_stage1 = 'd.*_male_pk', corr_stage2 = 'd.*_female_pk', sufx='ness')
```

```{r}
##VKCK male and femaleness correlation and (anti-)correlation
corr_clustr_venn(dset = stocha_pk_gns, corr_stage1 = 'd.*_male_vkck', corr_stage2 = 'd.*_female_vkck', sufx='ness')
```

```{r}
##VKCK male and femaleness correlation and (anti-)correlation
corr_clustr_venn(dset = stocha_pk_gns, corr_stage1 = 'd.*_male_', corr_stage2 = 'd.*_female_', sufx='ness')
```


```{r}
##all_days  CDB and gambranch correlation and (anti-)correlation
corr_clustr_venn(corr_stage1 = 'dir_all_da.*_female_in', corr_stage2 = 'dir_all_da.*_male_in', sufx='ness')
```


```{r}
##aall CDB and gambranch correlation and (anti-)correlation
corr_clustr_venn(corr_stage1 = 'di.*l_female_in_', corr_stage2 = 'di.*l_male_in_', sufx='ness')
```

```{r}
##PK all_days CDB correlation and (anti-)correlation
##Assess the overlap between genes based on correlation (or anti-corretion) in CDB to either femaleness or maleness in overal(all) and alldays
...
```{r}
##All DAYS gambracch independently
...
```{r}
##All gambracch independently
corr_clustr_venn(corr_stage1 = 'di.*l_female_in_gambranch', corr_stage2 = 'di.*l_male_in_gambranch', sufx='ness')
```


```{r}
##All_days gambracch independently
...
##All DAYS CDB and gambranch independently
# corr_clustr_venn1 <- function(corr_clustr, day){
#   #stocha_gns %>% filter(if_any(contains('drivers'), ~str_detect(.,'all_days'))) %>% filter((!! rlang::sym(corr_clustr)) == day) 
#   stocha_gns %>% filter(if_any(contains('drivers'), ~str_detect(.,'all_days'))) %>% filter(!is.na(!! rlang::sym(corr_clustr))) 
# 
#   }
# 
# # corr_clustr_venn('female_drivers_in_CDB')
# # corr_clustr_venn('female_drivers_in_gambranch')
# options(repr.plot.width=7, repr.plot.height=7)
```

```{r}
##All gambracch independently
corr_clustr_venn(corr_stage1 = 'di.*l__in', corr_stage2 = 'dir_all_d.*l_in', sufx='ness')
```
...
```{r}
##CDB (anti-)correlation
##Assess the overlap between genes based on correlation (or anti-correlation) in CDB to either femaleness or maleness in BF2

corr_clustr_venn_flex(tbl1 = dr_gns_all, 
                 tbl2 = dr_gns_all, 
                 tbl3 = dr_gns_all, 
                 tbl4 = dr_gns_all, 
                 dir1 = 'correlated', 
                 ...
                 corr_stage1 = 'dir_al.*_female_in_CDB', 
                 corr_stage2 = 'dir_al.*_female_in_CDB', 
                 corr_stage3 = 'dir_al.*_male_in_CDB', 
                 corr_stage4 ='dir_al.*_male_in_CDB', 
                 sufx='ness',
                 prefx='')
```
...
```{r}
##PK vs stocha gambranch anti-corr cells correlation only
corr_clustr_venn_flex(tbl1 = stocha_pk_gns, 
                 tbl2 = stocha_pk_gns, 
                 tbl3 = stocha_pk_gns, 
                 tbl4 = stocha_pk_gns, 
...
```{r}
##ALL DAYS VS All DAYS n cells correlation only
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 ...
                 dir3 = 'correlated', 
                 dir4 = 'correlated', 
                 corr_stage1 = 'dir_all_d.*_female_in_', 
                 corr_stage2 = 'di.*l_female_in_', 
                 corr_stage3 = 'dir_all_d.*_male_in_', 
                 corr_stage4 ='di.*l_male_in_', 
                 sufx='ness',
                 prefx='all ')
```
...
```{r}
##All DAYS VS BF2 positive correlation
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 tbl2 = dr_gns_all, 
                 tbl3 = stocha_gns, 
                 tbl4 = dr_gns_all, 
                 ...
                 dir4 = 'correlated', 
                 corr_stage1 = 'dir_all_d.*_female_in_', 
                 corr_stage2 = 'dir_all_d.*_female_in_', 
                 corr_stage3 = 'dir_all_d.*_male_in_', 
                 corr_stage4 ='dir_all_d.*_male_in_', 
                 sufx='ness',
                 prefx='BF2 ')
```

```{r}
##All DAYS VS BF2 negative correlation
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 tbl2 = dr_gns_all, 
                 tbl3 = stocha_gns, 
                 tbl4 = dr_gns_all, 
...
```{r}
##BFcombi positive VS BF2 negative correlation 
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 tbl2 = dr_gns_all, 
                 tbl3 = stocha_gns, 
                 tbl4 = dr_gns_all, 
...
```{r}
##BFcombi negative VS BF2 positive correlation 
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 tbl2 = dr_gns_all, 
                 tbl3 = stocha_gns, 
                 tbl4 = dr_gns_all, 
...
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 ...
                 dir3 = 'correlated', 
                 dir4 = 'correlated', 
                 corr_stage1 = 'dir_all_d.*_female_in_CDB', 
                 corr_stage2 = 'dir_all_d.*_male_in_CDB', 
                 corr_stage3 = 'dir_all_d.*_asexual_in_', 
                 corr_stage4 ='dir_all_d.*_gametocyte_in_', 
                 sufx='ness',
                 prefx='')
```
...
```{r}
##ALL & All DAYS gambranch CDB positive
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 tbl2 = stocha_gns, 
                 tbl3 = stocha_gns, 
                 tbl4 = stocha_gns, 
                 ...
                 dir3 = 'correlated', 
                 dir4 = 'correlated', 
                 corr_stage1 = 'dir_a.*_female_in_CDB', 
                 corr_stage2 = 'dir_a.*_female_in_gambranch', 
                 corr_stage3 = 'dir_a.*_male_in_CDB', 
                 corr_stage4 = 'dir_a.*_male_in_gambranch', 
                 sufx='ness',
                 prefx='gambranch ')
```
...
```{r}
##All DAYS gambranch CDB positive
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 tbl2 = stocha_gns, 
                 tbl3 = stocha_gns, 
                 tbl4 = stocha_gns, 
                 ...
                 dir4 = 'correlated', 
                 corr_stage1 = 'dir_all_d.*_female_in_CDB', 
                 corr_stage2 = 'dir_all_d.*_female_in_gambranch', 
                 corr_stage3 = 'dir_all_d.*_male_in_CDB', 
                 corr_stage4 = 'dir_all_d.*_male_in_gambranch', 
                 sufx='ness',
                 prefx='gambranch ')
```
...
corr_clustr_venn_flex(tbl1 = stocha_gns, 
                 ...
                 dir3 = 'correlated', 
                 dir4 = 'correlated', 
                 corr_stage1 = 'di.*l_female_in_CDB', 
                 corr_stage2 = 'di.*l_female_in_gambranch', 
                 corr_stage3 = 'di.*l_male_in_CDB', 
                 corr_stage4 = 'di.*l_male_in_gambranch', 
                 sufx='ness',
                 prefx='gambranch ')
```
...
```{r}
stocha_gns %>% filter(ID %in% api_ap2)
```

```{r}
stocha_corr_genes %>% map(. %>% filter(ID %in% c('PF3D7_0611200','PF3D7_0613800','PF3D7_0802100','PF3D7_1449500','PF3D7_1456000') & qval<0.001 & Timepoint=='all_days'))
```
...
```{r}
VBreg_functional %>% glimpse
```

```{r}
# stocha_dr_gns_VB_cluster <- stocha_gns %>% 
# left_join(., (VBreg_functional %>% dplyr::select(ID, 'Cluster')) , by = c('Gene ID' ='Gene ID')) %>% 
# column_to_rownames('Gene ID')%>%
# mutate(CDgam_all_days = case_when(str_detect(female_drivers_in_CDB, 'all_days') | str_detect(male_drivers_in_CDB, 'all_days') ~ 'Yes',
#                                       !is.na(female_drivers_in_CDB) | !is.na(male_drivers_in_CDB) ~ 'No'
#                                      ),
#        Bgam_all_days = case_when(str_detect(female_drivers_in_gambranch, 'all_days') | str_detect(male_drivers_in_gambranch, 'all_days') ~ 'Yes',
#                                       !is.na(female_drivers_in_gambranch) | !is.na(male_drivers_in_gambranch) ~ 'No'
#                                      ),
#        CDgam_all = case_when(str_detect(female_drivers_in_CDB, 'all$') | str_detect(male_drivers_in_CDB, 'all$') ~ 'Yes',
#                                       !is.na(female_drivers_in_CDB) | !is.na(male_drivers_in_CDB) ~ 'No'
#                                      ),
#        Bgam_all = case_when(str_detect(female_drivers_in_gambranch, 'all$') | str_detect(male_drivers_in_gambranch, 'all$') ~ 'Yes',
#                                       !is.na(female_drivers_in_gambranch) | !is.na(male_drivers_in_gambranch) ~ 'No'
#                                      ),
#        Gam_lReT_all_days = case_when(str_detect(early_gam_drivers_in_lReT, 'all_days') ~ 'Yes',
#                                       !is.na(early_gam_drivers_in_lReT) ~ 'No'
#                                      ),
#        Gam_lReT_all = case_when(str_detect(early_gam_drivers_in_lReT, 'all$') ~ 'Yes',
#                                       !is.na(early_gam_drivers_in_lReT) ~ 'No'
#                                      )
#       ) 
```
...
```{r}
VBreg_functional %>% colnames
```

```{r}
VBgam_vs_asexual %>% 
filter(`PlasmoDB ID` %in% (stocha_gns %>% filter(if_any(matches('dir.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID))) %>%
...
pheatmap(., 
         cluster_cols = F,
         show_rownames = F,
         annotation_row = stocha_gns %>% filter(if_any(matches('dir.*ale_in_CDB'), ~.== 'correlated') )  %>% column_to_rownames('ID') %>% dplyr::select(CDgam_sex))
```

```{r}
stocha_dr_gns_VB_cluster %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )
```
...
```{r}
VBgam_vs_asexual %>% 
filter(`PlasmoDB ID` %in% (stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') )  %>% pull(ID))) %>%
dplyr::select(`PlasmoDB ID`, starts_with('Day')) %>% 
column_to_rownames("PlasmoDB ID") %>%
as.matrix() %>%
pheatmap(., 
         cluster_cols = F, 
         show_rownames = F,
         annotation_row = stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') )  %>% column_to_rownames('ID') %>% dplyr::select(Bgam_sex))
```


```{r}
VBgam_vs_asexual %>% 
filter(`PlasmoDB ID` %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*_female_in_gambranch'), ~.== 'correlated') ) %>% pull(ID))) %>% 
...
```{r}
VBgam_vs_asexual %>% 
filter(`PlasmoDB ID` %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*_male_in_gambranch'), ~.== 'correlated') ) %>% pull(ID))) %>% 
pivot_longer(cols = starts_with('Day'), names_to = 'Days') %>% 
mutate(across(c('Days'), ~factor(.,levels = VBgam_vs_asexual %>% select(starts_with('Day')) %>% colnames()))) %>%
# group_by(`PlasmoDB ID`) %>%
ggplot(., aes(x = Days,  y = value, group = `PlasmoDB ID`, color = `PlasmoDB ID`)) + 
...
```{r}
VBgam_vs_asexual %>% 
filter(`PlasmoDB ID` %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*_male_in_gambranch'), ~.== 'anti-correlated') ) %>% pull(ID))) %>% 
pivot_longer(cols = starts_with('Day'), names_to = 'Days') %>% 
mutate(across(c('Days'), ~factor(.,levels = VBgam_vs_asexual %>% select(starts_with('Day')) %>% colnames()))) %>%
# group_by(`PlasmoDB ID`) %>%
ggplot(., aes(x = Days,  y = value, group = `PlasmoDB ID`, color = `PlasmoDB ID`)) + 
...
```{r}
VBgam_vs_asexual %>% 
filter(`PlasmoDB ID` %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*_gametocyte_in_lReT'), ~.== 'correlated') ) %>% filter(if_any(matches('dir_all_d.*_asexual_in_lReT'), ~.== 'anti-correlated') ) %>% pull(ID))) %>% 
pivot_longer(cols = starts_with('Day'), names_to = 'Days') %>% 
mutate(across(c('Days'), ~factor(.,levels = VBgam_vs_asexual %>% select(starts_with('Day')) %>% colnames()))) %>%
# group_by(`PlasmoDB ID`) %>%
ggplot(., aes(x = Days,  y = value, group = `PlasmoDB ID`, color = `PlasmoDB ID`)) + 
...
```{r}
VBgam_vs_asexual %>% 
filter(`PlasmoDB ID` %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*_gametocyte_in_lReT'), ~.== 'anti-correlated') ) %>% filter(if_any(matches('dir_all_d.*_asexual_in_lReT'), ~.== 'correlated') ) %>% pull(ID))) %>% 
pivot_longer(cols = starts_with('Day'), names_to = 'Days') %>% 
mutate(across(c('Days'), ~factor(.,levels = VBgam_vs_asexual %>% select(starts_with('Day')) %>% colnames()))) %>%
# group_by(`PlasmoDB ID`) %>%
ggplot(., aes(x = Days,  y = value, group = `PlasmoDB ID`, color = `PlasmoDB ID`)) + 
...
right_join(., stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) , by='ID')
```

```{r}
TraProt_bias_CDB %>%
dplyr::count(`Lasonder transcript sex bias`, CDgam_sex) %>%
ggplot(., aes(x = `Lasonder transcript sex bias`, y = n, fill = CDgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```

```{r}
TraProt_bias_CDB %>%
dplyr::count(`Tao protein sex bias`, CDgam_sex) %>%
ggplot(., aes(x = `Tao protein sex bias`, y = n, fill = CDgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```

```{r}
TraProt_bias_gambranch <- VBreg_functional %>% 
dplyr::select('ID','Female transcripts', 'Male transcripts', 'Female proteins', 'Male Proteins') %>%
dplyr::filter(if_any(-c('ID'), ~ !is.na(.))) %>%
mutate('Lasonder transcript sex bias' = case_when(`Female transcripts` == 'Yes'  & `Male transcripts` == 'Yes' ~ 'Both',
                                        `Female transcripts` == 'Yes' & is.na(`Male transcripts`) ~ 'Female',
      ...
      'Tao protein sex bias' = case_when(`Female proteins` == 'Yes'  & `Male Proteins` == 'Yes' ~ 'Both',
                                        ...
                                        is.na(`Female proteins`) & `Male Proteins` == 'Yes' ~ 'Male',
                                        is.na(`Female proteins`) & is.na(`Male Proteins`) ~ 'None')) %>%
right_join(., stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') ) , by='ID')
```
...
```{r}
stocha_dr_gns_VB %>% dplyr::select(ID, CDgam_sex, Bgam_sex, `Lasonder transcript bias`, `Tao protein bias`, Cluster)%>%
mutate(across(c('CDgam_sex', 'Bgam_sex'), ~replace_na(.,'None') )) %>%
    filter(!(Bgam_sex == 'Female,Male' | CDgam_sex == 'Female,Male' )) %>%
dplyr::count(CDgam_sex, Bgam_sex)
```

```{r}
print(GGally::ggpairs(
    stocha_dr_gns_VB %>% dplyr::select(ID, CDgam_sex, Bgam_sex, `Lasonder transcript bias`, `Tao protein bias`)%>%
mutate(across(c('CDgam_sex', 'Bgam_sex'), ~replace_na(.,'None') )) %>%
    filter(!(Bgam_sex == 'Female,Male' | CDgam_sex == 'Female,Male' )) %>%
    tibble::column_to_rownames("ID"),# %>%
...
```{r}
# print(GGally::ggpairs(stocha_corr_genes[[4]] %>%
#                           filter(!is.na(`Gene ID`) & qval < 0.005 ) %>%
#                             dplyr::select(Gene_ID, corr, Timepoint) %>%
#                             pivot_wider(names_from = Timepoint, values_from = c('corr')) %>%
#                             tibble::column_to_rownames("ID"),
#                           # dplyr::mutate_all(.funs = log1p),
#                           lower = list(continuous = GGally::wrap(
...
```{r}
TraProt_bias_gambranch %>% 
dplyr::count(`Lasonder transcript sex bias`, Bgam_sex) %>%
ggplot(., aes(x = `Lasonder transcript sex bias`, y = n, fill = Bgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```

```{r}
TraProt_bias_gambranch %>% 
dplyr::count(`Tao protein sex bias`, Bgam_sex) %>%
ggplot(., aes(x = `Tao protein sex bias`, y = n, fill = Bgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```
...
##ALL_DAYS
VBreg_functional %>% 
right_join(., stocha_gns %>% 
           filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ), by='ID') %>%
dplyr::count(CDgam_sex, Cluster) %>%
...
VBreg_functional %>% 
right_join(., stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') ) , by='ID') %>%
dplyr::count(Bgam_sex_all_days, Cluster) %>%
ggplot(., aes(x = Cluster, y = n, fill = Bgam_sex_all_days)) +
geom_col(position = position_dodge2(preserve = 'single'))
```

```{r}
VBreg_functional %>% 
right_join(., stocha_gns , by='ID') %>%
dplyr::count(Bgam_sex, Cluster) %>%
...
unite('Sex', CDgam_sex:Bgam_sex, na.rm = T)  %>%
mutate(across(Sex, ~case_when(str_detect(., 'Female,Male|Male_Female|Female_Male') ~ 'Female,Male',
                     str_detect(., 'Female') ~ 'Female',
                     str_detect(., 'Male') ~ 'Male'))) %>%
dplyr::count(Sex, Cluster,`Tao protein bias`,`Lasonder transcript bias`) %>%
...
stocha_dr_gns_VB %>% 
filter(if_any(contains('drivers'), ~str_detect(.,'all_days'))) %>% 
filter(CDgam_sex %in% c('Female','Male') | is.na(CDgam_sex)) %>%
filter(Bgam_sex %in% c('Female','Male') | is.na(Bgam_sex)) %>%
# unite('Sex', CDgam_sex:Bgam_sex, na.rm = T)  %>%
# mutate(across(Sex, ~case_when(str_detect(., 'Female,Male|Male_Female|Female_Male') ~ 'Female,Male',
#                      str_detect(., 'Female') ~ 'Female',
#                      str_detect(., 'Male') ~ 'Male'))) %>%
dplyr::count(CDgam_sex,Bgam_sex , Cluster,`Lasonder transcript bias`) %>%
...
mutate(across(Sex, ~case_when(str_detect(., 'Female,Male|Male_Female|Female_Male') ~ 'Female,Male',
                     str_detect(., 'Female') ~ 'Female',
                     str_detect(., 'Male') ~ 'Male'))) %>%
dplyr::count(Sex, Cluster,`Lasonder transcript bias`) %>%
ggplot(., aes(x = Cluster, y = n, fill = Sex)) +
geom_col(position = position_dodge2(preserve = 'single')) +
...
# #                      str_detect(., 'Female') ~ 'Female',
# #                      str_detect(., 'Male') ~ 'Male'))) %>%
# count(Sex, Cluster,`Tao protein bias`,`Lasonder transcript bias`) %>%
# ggplot(., aes(x = Cluster, y = n, fill = Sex)) +
# geom_col(position = position_dodge2(preserve = 'single')) +
# facet_grid(`Lasonder transcript bias`~`Tao protein bias`)
```
...
VBreg_functional %>% 
filter(Cluster == '8') %>%
left_join(., stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') ) , by='ID') %>%
dplyr::count(Bgam_sex, Cluster, `Male transcripts`) %>%
ggplot(., aes(x = `Male transcripts`, y = n, fill = Bgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```
...
VBreg_functional %>% 
filter(Cluster == '8') %>%
left_join(., stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) , by='ID') %>%
dplyr::count(CDgam_sex, Cluster, `Male transcripts`) %>%
ggplot(., aes(x = `Male transcripts`, y = n, fill = CDgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```
...
left_join(., stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) , by='ID')  %>%
dplyr::count(`Transcript sex bias`, CDgam_sex) %>%
ggplot(., aes(x = `Transcript sex bias`, y = n, fill = CDgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```
...
filter(if_any(-c('ID'), ~!is.na(.))) %>%
left_join(., stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') ) , by='ID')  %>%
dplyr::count(`Transcript sex bias`, Bgam_sex) %>%
ggplot(., aes(x = `Transcript sex bias`, y = n, fill = Bgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```

```{r}
stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) 
```

```{r}
VBreg_functional %>% 
filter(Cluster == '8' & `Male transcripts` == 'Yes') %>%
left_join(., stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) , by='ID') %>%
dplyr::count(Bgam_sex, Cluster) %>%
ggplot(., aes(x = Cluster, y = n, fill = Bgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
...
```{r}
# VBreg_functional %>% count(Cluster,`Male transcripts`)
```

```{r}
VBreg_functional %>% 
right_join(., stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) , by='ID') %>%
dplyr::select('ID','ATGTGTA motif','AGACA motif', 'CDgam_sex') %>%
dplyr::count(CDgam_sex, `ATGTGTA motif`, `AGACA motif`) 
```

```{r}
VBreg_functional %>% 
right_join(., stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') ) , by='ID') %>%
dplyr::select('ID','ATGTGTA motif','AGACA motif', 'Bgam_sex') %>%
dplyr::count(Bgam_sex, `ATGTGTA motif`, `AGACA motif`) 
```

```{r}
VBreg_functional %>% 
right_join(., stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') ) , by='ID') %>%
dplyr::select('ID','Other motifs', 'Bgam_sex') %>%
dplyr::count(Bgam_sex, `Other motifs`) 
```

```{r}
VBreg_functional %>% 
right_join(., stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') ) , by='ID') %>%
dplyr::select('ID',Bgam_sex, 'Stabilized transcripts') %>%
dplyr::count(Bgam_sex, `Stabilized transcripts`) 
```

```{r}
VBreg_functional %>% 
right_join(., stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') ) , by='ID') %>%
dplyr::select('ID',Bgam_sex, starts_with('AP2')) %>%
...
mutate(across(starts_with('AP2'), ~as.numeric(.))) %>%
dplyr::filter(if_any(starts_with('AP2'), ~ !is.na(.))) %>%
ggplot(., aes(x= Bgam_sex, y = `AP2_PF3D7_0611200`)) +
geom_boxplot()
```

```{r}
as.list(colnames(VBreg_functional)[VBreg_functional %>% colnames() %>% str_detect('Asex')])
```

```{r}
# VBreg_functional %>% 
# right_join(., stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) , by='ID') %>%
# dplyr::select('Gene_ID',Sex, starts_with('As')) %>%
# dplyr::filter(if_any(starts_with('As'), ~ !is.na(.))) %>%
# map(as.list(colnames(VBreg_functional)[VBreg_functional %>% colnames() %>% str_detect('Asex')]), 
#     ~ count(.data, Sex, .x) )
```

```{r}
colnames(VBreg_functional)[VBreg_functional %>% colnames() %>% str_detect('Asex')]
```

```{r}
# for (epi_marker in colnames(VBreg_functional)[VBreg_functional %>% colnames() %>% str_detect('Asex')]) {
#     print(epi_marker)
#     VBreg_functional %>% 
#     right_join(., stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) , by='ID') %>%
#     dplyr::select('Gene_ID',Sex, starts_with('As')) %>%
#     dplyr::filter(if_any(starts_with('As'), ~ !is.na(.))) %>%
#     count(Sex, !!epi_marker) %>%
#     ggplot(., aes(x = !!epi_marker, y = n, fill = Sex)) +
#     geom_col(position = position_dodge2(preserve = 'single'))
# }
...
VBreg_functional %>% 
right_join(., stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) , by='ID') %>%
dplyr::select('ID',CDgam_sex, starts_with('As')) %>%
dplyr::filter(if_any(starts_with('As'), ~ !is.na(.))) 
# %>%
...
dplyr::select('ID',CDgam_sex, starts_with('As')) %>%
dplyr::filter(if_any(starts_with('As'), ~ !is.na(.))) %>%
dplyr::count(CDgam_sex, `Asex_Epi_H3K36me3`) %>%
ggplot(., aes(x = `Asex_Epi_H3K36me3`, y = n, fill = CDgam_sex)) +
geom_col(position = position_dodge2(preserve = 'single'))
```
...
ggvenn(
  list(VB_early = VBreg_functional %>% filter(Cluster == '8' & `Male transcripts` == 'Yes') %>% pull(ID),
    male_dr_gambranch = stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') )  %>% pull(ID) ,
    male_dr_CDB = stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID) ,
...
right_join(., stocha_dr_gns_VB_cluster%>%
            ...
            # filter(is.na(Bgam_sex_all_days) | Bgam_sex_all_days != 'Female,Male')  %>% 
           filter(if_any(matches('dir_all_d.*ale_in_gambranch'), ~.== 'correlated') ) %>% 
           rownames_to_column('gene_id') , by='gene_id') %>% 
dplyr::select(gene_id,p_val,avg_log2FC,pct.1,pct.2,p_val_adj,SexDE, CDgam_sex,Bgam_sex_all_days) %>%
dplyr::count(SexDE, Bgam_sex_all_days) %>%
mutate_if(is.character, ~replace_na(.,'Not DE')) %>%
ggplot(., aes(x = SexDE, y = n, fill = Bgam_sex_all_days)) +
# geom_col(position = position_dodge2(preserve = 'single')) +
...
guides(fill=guide_legend(title="Sex correlation"))+ 
geom_col(position = position_dodge2(width = 0.2)) +
scale_fill_manual(values = dr_sex_col)+
geom_text(aes(label = n), position = position_dodge2(width = 0.8), vjust =0.5, hjust =0.8, size =8, angle = 90)
```
...
right_join(., stocha_dr_gns_VB_cluster %>%
           ...
           filter(if_any(matches('dir_all_d.*ale_in_gambranch'), ~.== 'correlated') ) %>% 
           rownames_to_column('gene_id') , by='gene_id') %>% 
dplyr::select(gene_id,p_val,avg_log2FC,pct.1,pct.2,p_val_adj,SexDE, CDgam_sex,Bgam_sex) %>%
dplyr::count(SexDE, Bgam_sex) %>%
mutate_if(is.character, ~replace_na(.,'Not DE')) %>%
ggplot(., aes(x = SexDE, y = n, fill = Bgam_sex)) +
# geom_col(position = position_dodge2(preserve = 'single')) +
theme(axis.text=element_text(size=26), 
      axis.title=element_text(size=30,face="bold"), 
      legend.text = element_text(size=30), 
...
right_join(., stocha_dr_gns_VB_cluster %>%
            # filter(is.na(CDgam_sex) | CDgam_sex != 'Female,Male') %>%
            filter(is.na(Bgam_sex) | Bgam_sex != 'Female,Male') %>% 
           filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) %>% 
           rownames_to_column('gene_id') , by='gene_id') %>% 
dplyr::select(gene_id,p_val,avg_log2FC,pct.1,pct.2,p_val_adj,SexDE, CDgam_sex) %>%
dplyr::count(SexDE, CDgam_sex) %>%
mutate_if(is.character, ~replace_na(.,'Not DE')) %>%
ggplot(., aes(x = SexDE, y = n, fill = CDgam_sex)) +
# geom_col(position = position_dodge2(preserve = 'single'))+
theme(axis.text=element_text(size=26), 
      axis.title=element_text(size=30,face="bold"), 
      legend.text = element_text(size=30), 
...
lfVSlm %>% mutate(SexDE = case_when(avg_log2FC > 0 ~ 'Female',
                                        avg_log2FC < 0 ~ 'Male')) %>%
rownames_to_column("gene_id") %>%
full_join(., Pf3D7_genes_plasmoDB[,c("gene_id", "Product Description", "Computed GO Components", "Computed GO Functions", 
"Computed GO Processes", "Curated GO Components", "Curated GO Functions", "Curated GO Processes")], by = "gene_id") %>%
mutate(across(gene_id, ~str_replace(., '-', '_') )) %>%
right_join(., stocha_dr_gns_VB_cluster %>%
            #filter(is.na(CDgam_sex) | CDgam_sex != 'Female,Male') %>%
...
```{r}
# sorted_gam_scale_data
```

```{r}
# gams@meta.data %>% dplyr::select(anno_300cells) %>% unique %>% pull %>% dput
```
...
```{r}
# stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID)
```

```{r}
# PfPDB_genes%>% 
# filter(gene_id %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID))) %>% column_to_rownames('gene_id')
```
...
# column_to_rownames('ID') %>%
# dplyr::select(Bgam_sex, SexDE) %>%
# rename('Sex correlation' = Bgam_sex)
```
...
```{r}
# stocha_dr_gns_VB %>%
# filter(ID %in% rownames(Bgam_annot)) %>% 
# filter(if_any(contains('drivers'), ~str_detect(.,'all_days'))) %>% 
# unite('Sex', CDgam_sex:Bgam_sex, na.rm = T)  %>%
# mutate(across(Sex, ~case_when(str_detect(., 'Female,Male|Male_Female|Female_Male') ~ 'Female,Male',
#                      str_detect(., 'Female') ~ 'Female',
...
mutate(across(SexDE, ~factor(replace_na(.,'Not DE'), levels = c('Female', 'Male', 'Not DE')))) %>%
column_to_rownames('ID') %>%
dplyr::select(CDgam_sex, SexDE) %>%
rename('Sex correlation' = CDgam_sex)
```

```{r}
##ONLY ALL_DAYS
asx_gam_annot <- stocha_gns%>% 
    # filter(if_any(contains('drivers'), ~str_detect(.,'all_days')))%>% 
  # filter(if_any(matches('dir_all_d.*gametocyte_in_lReT'), ~!is.na(.)) ) %>%
  filter(!is.na(lR_eT_Asex_gam_all_days) & !is.na(lR_eT_Asex_gam_all_days_anti)) %>%
    column_to_rownames('ID') %>% 
    dplyr::select(lR_eT_Asex_gam_all_days, lR_eT_Asex_gam_all_days_anti) %>%
    # mutate(across(lR_eT_Asex_gam, ~str_remove(.,'\\(schizont\\)'))) %>%
    rename('Stage correlation' = lR_eT_Asex_gam_all_days, 'Stage anti-correlation' = lR_eT_Asex_gam_all_days_anti)
```
...
sorted_gam_scale_data[stocha_gns %>% 
                      # filter(if_any(contains('drivers'), ~str_detect(.,'all_days')))%>% 
                      # filter(is.na(CDgam_sex) | CDgam_sex != 'Female,Male') %>%
                      # filter(is.na(CDgam_sex) | Bgam_sex != 'Female,Male') %>% 
                      filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) %>% 
                      pull(ID),] %>%
pheatmap(., 
         cluster_cols = F, 
         show_rownames = F,
         # labels_row = PfPDB_genes%>% filter(gene_id %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID)))  %>% pull(Gene),
        show_colnames = F,
...
```{r, fig.width= 12, fig.height=10}
# options(repr.plot.width=12, repr.plot.height=5)
sorted_asx_gam_scale_data[stocha_gns %>% 
                      # filter(if_any(contains('drivers'), ~str_detect(.,'all_days')))%>% 
                      # filter(is.na(CDgam_sex) | CDgam_sex != 'Female,Male') %>%
                      # filter(is.na(Bgam_sex) | Bgam_sex != 'Female,Male') %>% 
                      filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) %>% 
...
```{r, fig.width= 12, fig.height=10}
# options(repr.plot.width=12, repr.plot.height=5)
female_sorted_asx_gam_scale_data[stocha_gns %>% 
                      filter(if_any(contains('drivers'), ~str_detect(.,'all_days')))%>% 
                      # filter(is.na(CDgam_sex) | CDgam_sex != 'Female,Male') %>%
                      filter(is.na(Bgam_sex) | Bgam_sex != 'Female,Male') %>% 
                      filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) %>% 
...
mutate(across(SexDE, ~factor(replace_na(.,'Not DE'), levels = c('Female', 'Male', 'Not DE')))) %>% 
mutate(across(Bgam_sex_all_days, ~factor(replace_na(.,'No bias'), levels = c('Male', 'Female', 'Female,Male', 'No bias')))) %>% 
mutate(across(Bgam_sex_all_days_anti, ~factor(replace_na(.,'No bias'), levels = c('Male', 'Female', 'Female,Male', 'No bias')))) %>%
column_to_rownames('ID') %>%
dplyr::select(SexDE, Bgam_sex_all_days, Bgam_sex_all_days_anti) %>%
rename('Sex correlation' = Bgam_sex_all_days, 'Sex anti-correlation' = Bgam_sex_all_days_anti)
```
...
ggvenn(
  list(BF2_DE = BF2Res %>% arrange(desc(waldStat)) %>% head(n=500) %>% rownames() %>%str_replace(.,'-','_'),
    # male_dr_gambranch = stocha_gns %>% filter(if_any(matches('dir.*_male_in_gambranch'), ~.== 'correlated') )  %>% pull(ID) ,
...
# options(repr.plot.width=12, repr.plot.height=7)
sorted_gam_scale_data_ffirst2[stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID),] %>%
pheatmap(., 
         cluster_cols = F, 
         show_rownames = F,
         # labels_row = PfPDB_genes%>% filter(gene_id %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID)))  %>% pull(Gene),
        show_colnames = F,
         annotation_row = (stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% dplyr::select(CDgam_sex, `Gene ID`) %>% column_to_rownames('Gene ID'))%>% rename('Sex correlation' = CDgam_sex),
        annotation_col = sorted_gam_scale_data_ffirst_bcodes %>% rename('Gametocytes' = anno_300cells) %>%dplyr::select(Gametocytes),
        fontsize = 20)
# options(repr.plot.width=7, repr.plot.height=7)
```
...
```{r, fig.width= 12, fig.height=7}
options(repr.plot.width=12, repr.plot.height=7)
sorted_gam_scale_data_ffirst2[stocha_gns %>% filter(if_any(contains('drivers'), ~str_detect(.,'all$'))) %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') ) %>% pull(ID),] %>%
pheatmap(., 
         cluster_cols = F, 
         show_rownames = F,
         # labels_row = PfPDB_genes%>% filter(gene_id %in% (stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID)))  %>% pull(Gene),
...
```{r, fig.width= 12, fig.height=7}
options(repr.plot.width=12, repr.plot.height=7)
sorted_gam_scale_data[stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') )  %>% pull(ID),] %>%
pheatmap(., 
         cluster_cols = F, 
         show_rownames = F,
        show_colnames = F,
         annotation_row = stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') )  %>% column_to_rownames('Gene ID') %>% dplyr::select(Bgam_sex)%>% rename('Sex correlation' = Bgam_sex),
...
pheatmap(., 
         cluster_cols = F, 
         show_rownames = F,
         annotation_row = stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') )  %>% column_to_rownames('Gene ID') %>% dplyr::select(Bgam_sex))
```

```{r}
stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% arrange(CDgam_sex)
```
...
```{r}
stocha_gns %>% left_join(Josling_ap2g_stg1_gam %>% unite('id', c(left_id,right_id), sep = '_', na.rm = TRUE, remove = F) %>% pivot_longer(cols = c(left_id, right_id),names_to = 'relative_pos',values_to = 'ID', values_drop_na = T) , by = 'ID') %>% filter(!is.na(id)) %>%
...
```{r}
stocha_gns %>% left_join(Josling_ap2g_sc_ring %>% unite('id', c(left_id,right_id), sep = '_', na.rm = TRUE, remove = F) %>% pivot_longer(cols = c(left_id, right_id),names_to = 'relative_pos',values_to = 'ID', values_drop_na = T) , by = 'ID') %>% filter(!is.na(id)) %>%
    filter(if_any(contains('drivers'), ~str_detect(.,'all_days')))  %>% dplyr::count(relative_pos, CDgam_sex, lR_eT_Asex_gam) %>% ggplot(., aes(x = lR_eT_Asex_gam, y = n, fill = CDgam_sex)) +
    geom_col(position = position_dodge2(preserve = 'single'))+ 
    geom_text(aes(label = n), position = position_dodge2(width = 0.8), vjust =0.5, hjust =0.5, size =6)+
    facet_grid(.~relative_pos, labeller = labeller(.cols  = c('right_id' = 'right', 'left_id' = 'left'))) +
...
```{r}
stocha_gns %>% left_join(Josling_ap2g_ncc %>% unite('id', c(left_id,right_id), sep = '_', na.rm = TRUE, remove = F) %>% pivot_longer(cols = c(left_id, right_id),names_to = 'relative_pos',values_to = 'ID', values_drop_na = T) , by = 'ID') %>% filter(!is.na(id)) %>%
    filter(if_any(contains('drivers'), ~str_detect(.,'all_days')))  %>% dplyr::count(relative_pos, CDgam_sex, lR_eT_Asex_gam) %>% ggplot(., aes(x = lR_eT_Asex_gam, y = n, fill = CDgam_sex)) +
    geom_col(position = position_dodge2(preserve = 'single'))+ 
    geom_text(aes(label = n), position = position_dodge2(width = 0.8), vjust =0.5, hjust =0.5, size =6)+
    facet_grid(.~relative_pos, labeller = labeller(.cols  = c('right_id' = 'right', 'left_id' = 'left'))) +
...
options(repr.plot.width=21, repr.plot.height=10)
DoHeatmap(gams, features = stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% arrange(CDgam_sex) %>% pull(ID) %>% str_replace(., '_', '-')) + NoLegend()
# options(repr.plot.width=7, repr.plot.height=7)
```
...
```{r}
stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID) %>% str_replace(., '_', '-') %>% length
```

```{r}
# gams@assays$RNA@scale.data[stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID) %>% str_replace(., '_', '-'),]
```
...
pmap(
    ~ ..1[stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID),] %>%
    pheatmap(
        .,
        cluster_cols = F,
        show_rownames = F,
...
list(sorted_gam_scale_data_ffirst_list, sorted_gam_scale_data_ffirst_bcodes_list) %>%
pmap(
    ~ ..1[stocha_gns %>% filter(if_any(matches('dir.*ale_in_gambranch'), ~.== 'correlated') )  %>% pull(ID),] %>%
    pheatmap(
        .,
        cluster_cols = F,
        show_rownames = F,
...
#             female_dr_CDB = stocha_gns %>% filter(!is.na(female_drivers_in_CDB)  & CDgam_sex == 'Female') %>% pull(ID) ,
...
```{r}
stocha_gns %>% filter(!is.na(female_drivers_in_CDB) & CDgam_sex == 'Female') %>% pull(ID)
```
...
ggvenn(list('CITH KO DR' = CITH_KO_new_PfIDs %>% filter(`WT vs CITH` > 0) %>% pull(gene_id) %>% str_replace('-', '_'),
            female_dr_CDB = stocha_gns %>% filter(!is.na(female_drivers_in_CDB) & CDgam_sex == 'Female') %>% pull(ID),
            female_dr_gambranch = stocha_gns %>% filter(!is.na(female_drivers_in_gambranch) & Bgam_sex == 'Female') %>% pull(ID),
            'Genes with variable kinetics' = vkg_all %>% pull(`Gene_ID`)
           ),
       fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#A46868"),
       stroke_size = 0.5, set_name_size = 4
...
```{r}
enrichGO(stocha_gns %>% filter( !is.na(female_drivers_in_CDB) & CDgam_sex == 'Female') %>% pull(ID),universe=geneList[geneList %in% QCd_gene_4vel_unique], OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP") %>%
...
```{r}
dr_cdb_enr <- enrichGO(stocha_gns %>% filter(if_any(matches('dir_all_d.*ale_in_CDB'), ~.== 'correlated') )  %>% pull(ID),universe=geneList, OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP")
```
...
```{r}
enrichGO(stocha_gns %>% filter( !is.na(male_drivers_in_CDB) & CDgam_sex == 'Male') %>% pull(ID),universe=geneList[geneList %in% QCd_gene_4vel_unique], OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP")
```
...
```{r}
enrichGO(stocha_gns %>% filter( !is.na(male_drivers_in_CDB) & CDgam_sex == 'Male') %>% pull(ID),universe=geneList, OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP") 
```
...
```{r}
enrichGO(stocha_gns %>% filter( !is.na(female_drivers_in_CDB) & CDgam_sex == 'Female') %>% pull(ID),universe=geneList, OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP") %>%
dotplot(.,font.size=14,showCategory=4) + theme(legend.key.height = unit(2,'mm'))
```

```{r}
enrichGO(stocha_gns %>% filter( !is.na(male_drivers_in_gambranch) & Bgam_sex == 'Male') %>% pull(ID),universe=geneList, OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP") %>%
dotplot(.,font.size=14,showCategory=4) + theme(legend.key.height = unit(2,'mm'))
```

```{r}
enrichGO(stocha_gns %>% filter( !is.na(male_drivers_in_gambranch) & Bgam_sex == 'Male') %>% pull(ID),universe=geneList[geneList %in% QCd_gene_4vel_unique], OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP") %>%
dotplot(.,font.size=14,showCategory=4) + theme(legend.key.height = unit(2,'mm'))
```
...
```{r}
enrichGO(stocha_gns %>% filter( !is.na(female_drivers_in_gambranch) & Bgam_sex == 'Female') %>% pull(ID),universe=geneList, OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP") %>%
dotplot(.,font.size=14,showCategory=4) + theme(legend.key.height = unit(2,'mm'))
```

```{r}
enrichGO(stocha_gns %>% filter( !is.na(female_drivers_in_gambranch) & Bgam_sex == 'Female') %>% pull(ID),universe=geneList[geneList %in% QCd_gene_4vel_unique], OrgDb='org.Pf.plasmo.db', keyType ="SYMBOL",ont="BP") %>%
dotplot(.,font.size=14,showCategory=4) + theme(legend.key.height = unit(2,'mm'))
```
