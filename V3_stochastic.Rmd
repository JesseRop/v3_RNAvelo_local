...
```{r}
list.files("data/processed/PfDB52e/all", pattern = "scv_bf1_b4CR_stocha.h5ad", full.names = T, recursive = T, include.dirs = T)
```

```{r}
list.files("data/processed/PfDB52e/all_days/", pattern = "scv_..._stocha.h5ad", full.names = T, recursive = T, include.dirs = T)%>%
set_names(nm = (str_remove_all(.,'data/processed/PfDB52e/all_days/|/scv_|_stocha.h5ad')))
```


```{r}
ef_scv_stochastic <- list.files("data/processed/PfDB52e/all", pattern = "scv_bf1_b4CR_stocha.h5ad", full.names = T, recursive = T, include.dirs = T) %>%
set_names(nm = (str_remove_all(.,'data/processed/PfDB52e/|/scv_bf1_b4CR_stocha.h5ad'))) %>%
map(scv$read)
ef_scv_stocha_all <-scv$read("data/processed/PfDB52e/all/scv_bf1_b4CR_stocha.h5ad")
```
...
```{r, eval=FALSE}
asex_loom <- scv$read_loom('data/processed/PfDB52e/asexuals/vcyto/possorted_genome_bam_UZNXI.loom')
asex_loom$obs_names <- asex_loom$obs_names$to_list()  %>% str_replace('possorted_genome_bam_UZNXI:', 'D0_') %>%  str_replace('x', '-1') 

asex_loom_s = t(asex_loom[asex_loom$obs_names$to_list() %in% ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs_names$to_list()]$layers['spliced']$toarray())
asex_loom_us = t(asex_loom[asex_loom$obs_names$to_list() %in% ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs_names$to_list()]$layers['unspliced']$toarray())

```

```{r, eval=FALSE}
colnames(asex_loom_s) <- asex_loom[asex_loom$obs_names$to_list() %in% ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs_names$to_list()]$obs_names$to_list()
rownames(asex_loom_s) <- asex_loom[asex_loom$obs_names$to_list() %in% ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs_names$to_list()]$var_names$to_list()

colnames(asex_loom_us) <- asex_loom[asex_loom$obs_names$to_list() %in% ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs_names$to_list()]$obs_names$to_list()
rownames(asex_loom_us) <- asex_loom[asex_loom$obs_names$to_list() %in% ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs_names$to_list()]$var_names$to_list()
```

```{r, eval=FALSE}
## pull out PCA 
fit.quantile <- 0.1
pcs <- ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obsm['X_pca']
rownames(pcs) <- ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs_names$to_list()
cell.dist <- as.dist(1-cor(t(pcs)))

## filter genes
gexp1 <- log10(rowSums(asex_loom_s)+1)
gexp2 <- log10(rowSums(asex_loom_us)+1)
plot(gexp1, gexp2)

good.genes <- row.names(asex_loom_s)[gexp1 > 2 & gexp2 > 1]

gexp1_filt <- log10(rowSums(asex_loom_s[good.genes,])+1)
gexp2_filt <- log10(rowSums(asex_loom_us[good.genes,])+1)
plot(gexp1_filt, gexp2_filt)
...
```
```{r, eval=FALSE}
rvel.cd <- gene.relative.velocity.estimates(asex_loom_s[good.genes,], asex_loom_us[good.genes,],
                                            deltaT=1, kCells=30,
                                            cell.dist=cell.dist,
                                            fit.quantile=fit.quantile, 
                                            mult=100)

# rvel.qf <- gene.relative.velocity.estimates(asex_loom_s[1:1500,1:1500], asex_loom_us[1:1500,1:1500], deltaT=1, kCells = 5, fit.quantile = 0.02, min.nmat.emat.slope= 0.02, min.nmat.emat.correlation= 0.02, kGenes=5, n.cores = 6)
```
...
```{r, eval=FALSE}
pseudo_afm = Road2UMAP[,rvel.cd$cellKNN %>% colnames()]@meta.data %>% mutate(pseudo_afm = coalesce(pseudo_asex, pseudo_female, pseudo_male)) %>% select(pseudo_afm)
```

```{r, eval=FALSE}
up_s <- t(apply(up, 1, function(a) {a<- a-min(a); a <- a/max(a);}))
cell_up_s <- colSums(up_s)
plot(pseudo_afm$pseudo_afm, cell_up_s, xlab="pseudotime", ylab="increase in transcription")
...
```{r, eval=FALSE}
## get embedding
emb <-ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obsm['X_umap']
clusters <- ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs$Stage
rownames(emb) <- names(clusters) <- ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$obs_names$values
...
```{r}
# D0_asex_s <- ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$layers['spliced']$toarray()
# D0_asex_us <- ef_scv_stochastic[[1]][ef_scv_stochastic$all$obs$orig.ident == 'D0']$layers['unspliced']$toarray()

# asex_loom_s <- asex_loom$layers['spliced']$toarray()
# asex_loom_us <- asex_loom$layers['unspliced']$toarray()

```

```{r, eval=FALSE}
saveRDS(asex_loom_s, 'data/processed/PfDB52e/asexuals/vcyto/asex_loom_s.RDS')
saveRDS(asex_loom_us, 'data/processed/PfDB52e/asexuals/vcyto/asex_loom_us.RDS')
...
##Copy souporcell files for the optimum clusters
 ...
 system(
   'rsync -av /Volumes/GoogleDrive/My\\ Drive/PHD_documents/malaria_phd/sc_velo/data/processed/PfDB52e/asexuals/vcyto/asex_loom_s.RDS  jr35@farm5-head1:/lustre/scratch118/malaria/team222/jr35/phd/V3_4TP/sc_velo/data/processed/PfDB52e/asexuals/vcyto/'
          )
system(
  'rsync -av /Volumes/GoogleDrive/My\\ Drive/PHD_documents/malaria_phd/sc_velo/data/processed/PfDB52e/asexuals/vcyto/asex_loom_us.RDS  jr35@farm5-head1:/lustre/scratch118/malaria/team222/jr35/phd/V3_4TP/sc_velo/data/processed/PfDB52e/asexuals/vcyto/'
          )
```
...
```{r}
list.files("data/processed/RoadtoUMAP/", pattern = "_vkck_|_pk_", full.names = T, recursive = T, include.dirs = T)%>%
set_names(nm = (str_remove_all(.,'data/processed/RoadtoUMAP//|.csv|_bf1|_bf2')))
```

```{r}
stocha_pk_corr_genes <-list.files("data/processed/RoadtoUMAP/", pattern = "_vkck_|_pk_", full.names = T, recursive = T, include.dirs = T)%>%
set_names(nm = (str_remove_all(.,'data/processed/RoadtoUMAP//|.csv|_bf1|_bf2'))) %>%
map(read_csv) %>%
map(. %>%
    rename('ID' = 1)%>%
...
map(. %>%
    dplyr::filter(qval < 0.00005 & (corr > 0.3)) %>%
    arrange(desc(corr)) %>%
   pull(`ID`))
```
...
```{r}
DimPlot(Road2UMAP, reduction = "umap", dims = c(2,3), group.by= 'Stage', label=T)
```
...
stocha_pk_gns <- stocha_pk_corr_genes %>% 
  ...
  mutate(
    Bgam_sex_stocha_VKCK_bf = case_when(
      if_any(matches('d.*_female_drivers_vkck_bf'), ~.== 'correlated') & if_any(matches('d.*_male_drivers_vkck_bf'), ~.== 'correlated') ~ 'Female,Male',
      if_any(matches('d.*_female_drivers_vkck_bf'), ~.== 'correlated') ~'Female',
      if_any(matches('di.*_male_drivers_vkck_bf'), ~.== 'correlated') ~ 'Male'
      ),
    Asex_gam_stocha_VKCK_bf = case_when(
      if_any(matches('d.*_asexual_drivers_vkck_bf'), ~.== 'correlated') & if_any(matches('d.*_early_gam_drivers_vkck_bf'), ~.== 'correlated') ~ 'Asexual,Gametocyte',
      if_any(matches('d.*_asexual_drivers_vkck_bf'), ~.== 'correlated')  ~ 'Asexual',
      if_any(matches('d.*_early_gam_drivers_vkck_bf'), ~.== 'correlated') ~ 'Gametocyte'
      ),
    Bgam_sex_stocha_VKCK_anti_bf = case_when(
      if_any(matches('d.*_female_drivers_vkck_bf'), ~.== 'anti-correlated') & if_any(matches('d.*_male_drivers_vkck_bf'), ~.== 'anti-correlated') ~ 'Female,Male',
      if_any(matches('d.*_female_drivers_vkck_bf'), ~.== 'anti-correlated')  ~ 'Female',
      if_any(matches('d.*_male_drivers_vkck_bf'), ~.== 'anti-correlated') ~ 'Male'
      ),
    Asex_gam_stocha_VKCK_anti_bf = case_when(
      if_any(matches('d.*_asexual_drivers_vkck_bf'), ~.== 'anti-correlated') & if_any(matches('d.*_early_gam_drivers_vkck_bf'), ~.== 'anti-correlated') ~ 'Asexual,Gametocyte',
      if_any(matches('d.*_asexual_drivers_vkck_bf'), ~.== 'anti-correlated') ~ 'Asexual',
      if_any(matches('d.*_early_gam_drivers_vkck_bf'), ~.== 'anti-correlated')  ~ 'Gametocyte'
      ),
    Bgam_sex_PK_bf = case_when(
      if_any(matches('d.*_female_drivers_pk_bf'), ~.== 'correlated') & if_any(matches('d.*_male_drivers_pk_bf'), ~.== 'correlated') ~ 'Female,Male',
      if_any(matches('d.*_female_drivers_pk_bf'), ~.== 'correlated')  ~ 'Female',
      if_any(matches('d.*_male_drivers_pk_bf'), ~.== 'correlated')  ~ 'Male'
      ),
    Asex_gam_PK_bf = case_when(
      if_any(matches('d.*_asexual_drivers_pk_bf'), ~.== 'correlated') & if_any(matches('d.*_early_gam_drivers_pk_bf'), ~.== 'correlated') ~ 'Asexual,Gametocyte',
      if_any(matches('d.*_asexual_drivers_pk_bf'), ~.== 'correlated')  ~ 'Asexual',
      if_any(matches('d.*_early_gam_drivers_pk_bf'), ~.== 'correlated')  ~ 'Gametocyte'
      ),
    Bgam_sex_PK_anti_bf = case_when(
      if_any(matches('d.*_female_drivers_pk_bf'), ~.== 'anti-correlated') & if_any(matches('d.*_male_drivers_pk_bf'), ~.== 'anti-correlated') ~ 'Female,Male',
      if_any(matches('d.*_female_drivers_pk_bf'), ~.== 'anti-correlated')  ~ 'Female',
      if_any(matches('d.*_male_drivers_pk_bf'), ~.== 'anti-correlated')  ~ 'Male'
      ),
    Asex_gam_PK_anti_bf = case_when(
      if_any(matches('d.*_asexual_drivers_pk_bf'), ~.== 'anti-correlated') & if_any(matches('d.*_early_gam_drivers_pk_bf'), ~.== 'anti-correlated') ~ 'Asexual,Gametocyte',
      if_any(matches('d.*_asexual_drivers_pk_bf'), ~.== 'anti-correlated')  ~ 'Asexual',
      if_any(matches('d.*_early_gam_drivers_pk_bf'), ~.== 'anti-correlated')  ~ 'Gametocyte'
      ),
    Bgam_sex_stocha_VKCK = case_when(
      if_any(matches('d.*_female_drivers_vkck_all'), ~.== 'correlated') & if_any(matches('d.*_male_drivers_vkck_all'), ~.== 'correlated') ~ 'Female,Male',
      if_any(matches('d.*_female_drivers_vkck_all'), ~.== 'correlated') ~'Female',
      if_any(matches('di.*_male_drivers_vkck_all'), ~.== 'correlated') ~ 'Male'
      ),
    Asex_gam_stocha_VKCK = case_when(
      if_any(matches('d.*_asexual_drivers_vkck_all'), ~.== 'correlated') & if_any(matches('d.*_early_gam_drivers_vkck_all'), ~.== 'correlated') ~ 'Asexual,Gametocyte',
      if_any(matches('d.*_asexual_drivers_vkck_all'), ~.== 'correlated')  ~ 'Asexual',
      if_any(matches('d.*_early_gam_drivers_vkck_all'), ~.== 'correlated') ~ 'Gametocyte'
      ),
    Bgam_sex_stocha_VKCK_anti = case_when(
      if_any(matches('d.*_female_drivers_vkck_all'), ~.== 'anti-correlated') & if_any(matches('d.*_male_drivers_vkck_all'), ~.== 'anti-correlated') ~ 'Female,Male',
      if_any(matches('d.*_female_drivers_vkck_all'), ~.== 'anti-correlated')  ~ 'Female',
      if_any(matches('d.*_male_drivers_vkck_all'), ~.== 'anti-correlated') ~ 'Male'
      ),
    Asex_gam_stocha_VKCK_anti = case_when(
      if_any(matches('d.*_asexual_drivers_vkck_all'), ~.== 'anti-correlated') & if_any(matches('d.*_early_gam_drivers_vkck_all'), ~.== 'anti-correlated') ~ 'Asexual,Gametocyte',
      if_any(matches('d.*_asexual_drivers_vkck_all'), ~.== 'anti-correlated') ~ 'Asexual',
      if_any(matches('d.*_early_gam_drivers_vkck_all'), ~.== 'anti-correlated')  ~ 'Gametocyte'
      ),
    Bgam_sex_PK = case_when(
      if_any(matches('d.*_female_drivers_pk_all'), ~.== 'correlated') & if_any(matches('d.*_male_drivers_pk_all'), ~.== 'correlated') ~ 'Female,Male',
      if_any(matches('d.*_female_drivers_pk_all'), ~.== 'correlated')  ~ 'Female',
      if_any(matches('d.*_male_drivers_pk_all'), ~.== 'correlated')  ~ 'Male'
      ),
    Asex_gam_PK = case_when(
      if_any(matches('d.*_asexual_drivers_pk_all'), ~.== 'correlated') & if_any(matches('d.*_early_gam_drivers_pk_all'), ~.== 'correlated') ~ 'Asexual,Gametocyte',
      if_any(matches('d.*_asexual_drivers_pk_all'), ~.== 'correlated')  ~ 'Asexual',
      if_any(matches('d.*_early_gam_drivers_pk_all'), ~.== 'correlated')  ~ 'Gametocyte'
      ),
    Bgam_sex_PK_anti = case_when(
      if_any(matches('d.*_female_drivers_pk_all'), ~.== 'anti-correlated') & if_any(matches('d.*_male_drivers_pk_all'), ~.== 'anti-correlated') ~ 'Female,Male',
      if_any(matches('d.*_female_drivers_pk_all'), ~.== 'anti-correlated')  ~ 'Female',
      if_any(matches('d.*_male_drivers_pk_all'), ~.== 'anti-correlated')  ~ 'Male'
      ),
    Asex_gam_PK_anti = case_when(
      if_any(matches('d.*_asexual_drivers_pk_all'), ~.== 'anti-correlated') & if_any(matches('d.*_early_gam_drivers_pk_all'), ~.== 'anti-correlated') ~ 'Asexual,Gametocyte',
      if_any(matches('d.*_asexual_drivers_pk_all'), ~.== 'anti-correlated')  ~ 'Asexual',
      if_any(matches('d.*_early_gam_drivers_pk_all'), ~.== 'anti-correlated')  ~ 'Gametocyte'
      )
          ) %>%
  dplyr::rename_all(~str_replace_all(.,"male_drivers", "male")) %>%
  dplyr::rename_all(~str_replace_all(.,"early_schizont_drivers", "asexual")) %>%
...
```{r}
corr_ggally <- function(dset= stocha_pk_gns, mthds='co.*male_vkck'){

  print(GGally::ggpairs(dset %>%
                            dplyr::select(matches(mthds), ID) %>%
...
corr_ggally(dset= stocha_pk_gns,  mthds='co.*male_vkck')
corr_ggally(dset= stocha_pk_gns,  mthds='co.*male_pk')
corr_ggally(dset= stocha_pk_gns,  mthds='co.*male.*_all')
corr_ggally(dset= stocha_pk_gns,  mthds='co.*male.*_bf')
```

```{r}
##regex https://stackoverflow.com/questions/23518325/how-to-extract-substring-between-patterns-and-in-r 
corr_clustr_venn <- function(dset= stocha_pk_gns, corr_stage1 = 'dir_a.*_female_in_CDB', corr_stage2 = 'dir_a.*_male_in_CDB', sufx='ness'){
...
for (corr_clustr in c('_all', '_bf')) {
  print(corr_clustr_venn(dset=stocha_pk_gns, corr_stage1 = paste0('d.*_asexual_pk', corr_clustr), corr_stage2 = paste0('d.*_gametocyte_pk', corr_clustr), sufx='ness'))
  print(corr_clustr_venn(dset=stocha_pk_gns, corr_stage1 = paste0('d.*_asexual_vkck', corr_clustr), corr_stage2 = paste0('d.*_gametocyte_vkck', corr_clustr), sufx='ness'))
}
...
```{r}
## check one gene predicted to correlate to all lineages at bf1 when combining all variations (bf1,all,pk,vk)
Reduce(intersect, list((stocha_pk_gns %>% filter(if_any(matches('d.*_asexual_'), ~.== 'anti-correlated') ) %>% pull(ID)), (stocha_pk_gns %>% filter(if_any(matches('d.*_gametocyte_'), ~.== 'correlated') ) %>% pull(ID)),(stocha_pk_gns %>% filter(if_any(matches('d.*_asexual_'), ~.== 'correlated') ) %>% pull(ID)),(stocha_pk_gns %>% filter(if_any(matches('d.*_gametocyte_'), ~.== 'anti-correlated') ) %>% pull(ID)) ))
```
...
for (corr_clustr in c('_all', '_bf')) {
  print(corr_clustr_venn(dset=stocha_pk_gns, corr_stage1 = paste0('d.*_female_pk', corr_clustr), corr_stage2 = paste0('d.*_male_pk', corr_clustr), sufx='ness'))
  print(corr_clustr_venn(dset=stocha_pk_gns, corr_stage1 = paste0('d.*_female_vkck', corr_clustr), corr_stage2 = paste0('d.*_male_vkck', corr_clustr), sufx='ness'))
}
```

```{r}
###REMEMBER TO CHECK THESE GENES LATER
## check 23 genes predicted to correlate to all lineages at bf1 when combining all variations (bf1,all,pk,vk)
ambigous_gns <-Reduce(intersect, list((stocha_pk_gns %>% filter(if_any(matches('d.*_female_'), ~.== 'anti-correlated') ) %>% pull(ID)), (stocha_pk_gns %>% filter(if_any(matches('d.*_male_'), ~.== 'correlated') ) %>% pull(ID)),(stocha_pk_gns %>% filter(if_any(matches('d.*_female_'), ~.== 'correlated') ) %>% pull(ID)),(stocha_pk_gns %>% filter(if_any(matches('d.*_male_'), ~.== 'anti-correlated') ) %>% pull(ID)) ))

stocha_pk_gns %>% filter(ID %in% ambigous_gns) %>% select(matches('d.*male_|c.*male_')) %>% View()
...
for (corr_clustr in c('_all', '_bf')) {
  ...
  print(corr_clustr_venn_flex(tbl1 = stocha_pk_gns, 
                 tbl2 = stocha_pk_gns, 
                 tbl3 = stocha_pk_gns, 
                 tbl4 = stocha_pk_gns, 
                 dir1 = 'correlated', 
                 ...
                 dir3 = 'correlated', 
                 dir4 = 'correlated', 
                 corr_stage1 = paste0('d.*_female_vkck', corr_clustr), 
                 corr_stage2 = paste0('d.*_female_pk', corr_clustr), 
                 corr_stage3 = paste0('d.*_male_vkck', corr_clustr), 
                 corr_stage4 =paste0('d.*_male_pk', corr_clustr), 
                 sufx='ness',
                 prefx='PK '))
...
for (corr_clustr in c('_all', '_bf')) {
  ...
  print(corr_clustr_venn_flex(tbl1 = stocha_pk_gns, 
                 ...
                 dir3 = 'correlated', 
                 dir4 = 'correlated', 
                 corr_stage1 = paste0('d.*_gametocyte_vkck', corr_clustr), 
                 corr_stage2 = paste0('d.*_gametocyte_pk', corr_clustr), 
                 corr_stage3 = paste0('d.*_asexual_vkck', corr_clustr), 
                 corr_stage4 =paste0('d.*_asexual_pk', corr_clustr), 
                 sufx='ness',
                 prefx='PK '))
...
for (corr_clustr in c('_all', '_bf')) {
  ...
  print(corr_clustr_venn_flex(tbl1 = stocha_pk_gns, 
                 ...
                 dir4 = 'correlated', 
                 corr_stage1 = paste0('d.*_gametocyte_vkck', corr_clustr), 
                 corr_stage2 = paste0('d.*_asexual_vkck', corr_clustr), 
                 corr_stage3 = paste0('d.*_female_vkck', corr_clustr), 
                 corr_stage4 =paste0('d.*_male_vkck', corr_clustr), 
                 sufx='ness'))
...
```{r}
stocha_pk_gns %>% filter(dir_asexual_vkck_bf == 'correlated' & dir_male_vkck_bf == 'correlated') %>% select(dir_asexual_vkck_bf, dir_male_vkck_bf, corr_asexual_vkck_bf, corr_male_vkck_bf,dir_asexual_pk_bf,corr_asexual_pk_bf)
```
...
for (corr_clustr in c('_all', '_bf')) {
  ...
  print(corr_clustr_venn_flex(tbl1 = stocha_pk_gns, 
                 ...
                 dir3 = 'correlated', 
                 dir4 = 'correlated', 
                 corr_stage1 = paste0('d.*_gametocyte_pk', corr_clustr), 
                 corr_stage2 = paste0('d.*_asexual_pk', corr_clustr), 
                 corr_stage3 = paste0('d.*_female_pk', corr_clustr), 
                 corr_stage4 =paste0('d.*_male_pk', corr_clustr), 
                 sufx='ness'))
...
```{r}
sunil_DEU <- read_csv('sunil_isoseq/deu_genes.csv')
```


```{r}
stocha_pk_gns%>% filter(ID %in% sunil_DEU$`ID DEU`)
```


```{r}
stocha_pk_gns%>% filter(ID %in% sunil_DEU)
```
...
```
```{r}
stocha_pk_gns %>% filter(ID %in% early_gam)
```

```{r}
stocha_pk_gns %>% filter(ID %in% juli_markers)
```
```{r}
stocha_pk_gns %>% filter(ID %in% andy_markers)
```
...
```{r}
##BF2
vb_hmaps <- function(dset, corr_stage) {
  VBgam_vs_asexual %>% 
  filter(`PlasmoDB ID` %in% (dset %>% filter(!is.na({{corr_stage}}))  %>% pull(ID))) %>%
  ...
  pheatmap::pheatmap(., 
           ...
           show_rownames = F,
           annotation_row = dset %>% 
             filter(!is.na({{corr_stage}}))  %>% 
             column_to_rownames('ID') %>% 
             # mutate(annot= coalesce({{corr_stage}})) %>% 
             dplyr::select({{corr_stage}}))
}
```

```{r}
vb_hmaps(dset = stocha_pk_gns, corr_stage = 'Bgam_sex_stocha_VKCK')
vb_hmaps(dset = stocha_pk_gns, corr_stage = 'Bgam_sex_PK')
vb_hmaps(dset = stocha_pk_gns, corr_stage = 'Bgam_sex_stocha_VKCK_bf')
vb_hmaps(dset = stocha_pk_gns, corr_stage = 'Bgam_sex_PK_bf')
```
...
vb_hmaps <- function(dset, corr_stage) {
    VBgam_vs_asexual %>% 
    filter(`PlasmoDB ID` %in% (dset %>% 
                                 filter(if_any(matches(corr_stage), ~ !is.na(.))) %>% 
                                 pull(ID))) %>%
    dplyr::select(`PlasmoDB ID`, starts_with('Day')) %>% 
    column_to_rownames("PlasmoDB ID") %>%
    as.matrix() %>%
    pheatmap::pheatmap(., 
           ...
           annotation_row = dset %>% 
             filter(if_any(matches(corr_stage), ~ !is.na(.))) %>%
             column_to_rownames('ID') %>% 
             dplyr::select(matches(corr_stage)) %>%
             mutate(Sex = coalesce(!!!.))%>%
             select(Sex))
...
##Asex_gam_stocha_VKCK & Asex_gam_PK
vb_hmaps(dset = stocha_pk_gns, corr_stage = 'Asex_gam_.*k$')
```


```{r}
vb_lines <- function(dset, corr_stage, sex) {
    VBgam_vs_asexual %>% 
    filter(`PlasmoDB ID` %in% (dset %>% 
                                 filter(if_any(matches(corr_stage), ~.==sex)) %>% 
                                 pull(ID))) %>%
    pivot_longer(cols = starts_with('Day'), names_to = 'Days') %>% 
    mutate(across(c('Days'), ~factor(.,levels = VBgam_vs_asexual %>% 
...
```{r}
##Stochastic Female genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Bgam_sex_stocha_VKCK$', sex='Female')

##Stochastic Male genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Bgam_sex_stocha_VKCK$', sex='Male')

##Early Stochastic Female genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Bgam_sex_stocha_VKCK_bf$', sex='Female')

##Early Stochastic Male genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Bgam_sex_stocha_VKCK_bf$', sex='Male')

##Early PK Female genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Bgam_sex_PK_bf$', sex='Female')

##Early PK Male genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Bgam_sex_PK_bf$', sex='Male')

##Asexual stochastic Asexual genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Asex_gam_stocha_VKCK$', sex='Asexual')

##Gam stochastic Gametocyte genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Asex_gam_stocha_VKCK$', sex='Gametocyte')

##Asexual PK Asexual genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Asex_gam_PK$', sex='Asexual')

##Gam PK Gametocyte genes in VB
vb_lines(dset=stocha_pk_gns, corr_stage='Asex_gam_PK$', sex='Gametocyte')
```
...
```{r}
protra_bias <- function(corr_stage, trapro_b) {
    TraProt_bias_CDB %>% 
    filter(!is.na({{corr_stage}})) %>%
    dplyr::count({{trapro_b}}, {{corr_stage}}) %>%
    ggplot(., aes(x = {{trapro_b}}, y = n, fill = {{corr_stage}})) +
...
protra_bias <- function(corr_stage, trapro_b) {
    TraProt_bias_CDB %>% 
    filter(!is.na(.data[[corr_stage]])) %>%
    dplyr::count(.data[[trapro_b]], .data[[corr_stage]]) %>%
    ggplot(., aes(x =.data[[trapro_b]], y = n, fill = .data[[corr_stage]])) +
    geom_col(position = position_dodge2(preserve = 'single'))
}
```

```{r}
##Comparing where our genes are in lasonder tao genes
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', trapro_b='Lasonder transcript sex bias')
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', trapro_b='Tao protein sex bias')
```
...
```{r}
##BF1
##Comparing where lasonder tao genes are in our genes
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', trapro_b='Lasonder transcript bias')
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', trapro_b='Tao protein bias')

##our genes in VB clusters
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', trapro_b='Cluster')

##BF2
##Comparing where lasonder tao genes are in our genes
protra_bias(corr_stage='Asex_gam_stocha_VKCK', trapro_b='Lasonder transcript bias')
protra_bias(corr_stage='Asex_gam_stocha_VKCK', trapro_b='Tao protein bias')

##our genes in VB clusters
protra_bias(corr_stage='Asex_gam_stocha_VKCK', trapro_b='Cluster')
```

```{r}
protra_bias <- function(corr_stage, trapro_b, trapro_b2) {
    stocha_dr_gns_VB %>% 
    filter(!is.na(.data[[corr_stage]])) %>%
    dplyr::count(.data[[trapro_b]],.data[[trapro_b2]], .data[[corr_stage]]) %>%
    ggplot(., aes(x =.data[[trapro_b]], y = n, fill = .data[[corr_stage]])) +
    geom_col(position = position_dodge2(preserve = 'single'))+
    facet_grid(. ~ .data[[trapro_b2]])
}
```

```{r}
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', trapro_b='Cluster', trapro_b2='Lasonder transcript bias') 
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', trapro_b='Cluster', trapro_b2='Tao protein bias') 
protra_bias(corr_stage='Bgam_sex_PK', trapro_b='Cluster', trapro_b2='Lasonder transcript bias') 
protra_bias(corr_stage='Bgam_sex_PK_bf', trapro_b='Cluster', trapro_b2='Lasonder transcript bias') 
protra_bias(corr_stage='Bgam_sex_PK_bf', trapro_b='Cluster', trapro_b2='Male transcripts')
```
...
```{r}
protra_bias <- function(corr_stage, selectn) {
    VBreg_functional %>% 
    right_join(., stocha_pk_gns %>% filter(!is.na(.data[[corr_stage]])) , by='ID') %>%
    dplyr::select(matches(selectn))  %>%
    dplyr::count(., !!!.) 
}
```

```{r}
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', selectn='Bgam_sex_stocha_VKCK$|ATGTGTA motif|AGACA motif|Other motifs')
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', selectn='Bgam_sex_stocha_VKCK$|Stabilized transcripts')
```


```{r}
protra_bias(corr_stage='Bgam_sex_stocha_VKCK', selectn='Bgam_sex_stocha_VKCK$|^AP2')
protra_bias(corr_stage='Asex_gam_PK', selectn='Asex_gam_PK$|^AP2')
```

```{r}
h_methylatn<- function(corr_stage) {
    VBreg_functional %>% 
    right_join(., stocha_pk_gns %>% 
                 filter(!is.na(.data[[corr_stage]])) , by='ID') %>%
    dplyr::select('ID',.data[[corr_stage]], starts_with('As')) %>%
    dplyr::filter(if_any(starts_with('As'), ~ !is.na(.))) %>%
    dplyr::count(.data[[corr_stage]], `Asex_Epi_H3K36me3`) %>%
    ggplot(., aes(x = `Asex_Epi_H3K36me3`, y = n, fill = .data[[corr_stage]])) +
    geom_col(position = position_dodge2(preserve = 'single'))
}
```
...
```{r}
h_methylatn(corr_stage='Bgam_sex_stocha_VKCK')
h_methylatn(corr_stage='Bgam_sex_PK')
h_methylatn(corr_stage='Asex_gam_PK')
```
...
```{r}
dr_sex_col3 <- brewer.pal(3,"Set1")
names(dr_sex_col3) <- c('Male', 'Female', 'No bias')
```
...
dr_stage_col2 <- brewer.pal(3,"Set3")
names(dr_stage_col2) <- c('Asexual', 'Gametocyte', 'No bias')
```

```{r}
##ALL DAYS Bgam
lfVSlm <- lfVSlm %>% mutate(SexDE = case_when(avg_log2FC > 0 ~ 'Female',
                                        avg_log2FC < 0 ~ 'Male'))
```

```{r}
de_plots<- function(corr_stage,corr_stage1) {
  lfVSlm %>%
    rownames_to_column("gene_id") %>%
    mutate(across(gene_id, ~str_replace(., '-', '_') )) %>%
    right_join(., stocha_dr_gns_VB_cluster %>%
                 filter(if_any(matches(corr_stage1), ~ !is.na(.))) %>%
                 rownames_to_column('gene_id') , by='gene_id') %>%
    dplyr::select(gene_id,p_val,avg_log2FC,pct.1,pct.2,p_val_adj,SexDE,{{corr_stage}}) %>%
    dplyr::count(SexDE, {{corr_stage}}) %>%
    mutate_if(is.character, ~replace_na(.,'Not DE')) %>%
    ggplot(., aes(x = SexDE, y = n, fill = {{corr_stage}})) +
...
```{r}
de_plots(corr_stage='Bgam_sex_stocha_VKCK',corr_stage1='Bgam_sex_stocha_VKCK$')
de_plots(corr_stage='Asex_gam_stocha_VKCK',corr_stage1='Asex_gam_stocha_VKCK$') 
```
...
asx_gams@meta.data <- asx_gams@meta.data %>% 
  ...
  dplyr::select(anno_300cells, rank2, rank, rank3,stage_bfs,asx_gam_short_labs, pseudo_asex, pseudo_male, pseudo_female,seurat_clusters, RNA_snn_res.0.2, Stage)%>% 
...
```{r}
sorted_asx_gam_scale_data_ffirst_bcodes %>% dplyr::count(Stage, phmap_colors) %>% mutate_if(is.factor, as.character) %>% deframe
```
...
CDgam_annot <- stocha_pk_gns %>%
    ...
    mutate(across(SexDE, ~factor(replace_na(.,'Not DE'), levels = c('Female', 'Male', 'Not DE'))),
           across(Bgam_sex_stocha_VKCK_bf, ~replace_na(.,'No bias'))) %>%
    column_to_rownames('ID') %>%
dplyr::select(Bgam_sex_stocha_VKCK, SexDE, Bgam_sex_stocha_VKCK_bf)%>%
    rename('Sex correlation' = Bgam_sex_stocha_VKCK, 'Early sex correlation'=Bgam_sex_stocha_VKCK_bf)
```

```{r}
##ONLY ALL_DAYS
CDgam_annot_bf <- stocha_pk_gns %>%
    filter(!is.na(Bgam_sex_stocha_VKCK_bf)) %>%
    left_join(., lfVSlm %>% 
                  rownames_to_column("gene_id")  %>%
                  mutate(SexDE = case_when(avg_log2FC > 0 ~ 'Female',
                                           avg_log2FC < 0 ~ 'Male'),
    ...
    mutate(across(SexDE, ~factor(replace_na(.,'Not DE'), levels = c('Female', 'Male', 'Not DE'))),
           across(Bgam_sex_stocha_VKCK, ~replace_na(.,'No bias'))) %>%
    column_to_rownames('ID') %>%
dplyr::select(Bgam_sex_stocha_VKCK, SexDE, Bgam_sex_stocha_VKCK_bf)%>%
    rename('Sex correlation' = Bgam_sex_stocha_VKCK, 'Early sex correlation'=Bgam_sex_stocha_VKCK_bf)
```
...
```{r}
##ONLY ALL_DAYS
gam_asex_annot <- stocha_pk_gns %>%
    filter(if_any(matches('stocha_VKCK'), ~ !is.na(.))) %>%
    left_join(., lfVSlm %>% 
                  rownames_to_column("gene_id")  %>%
                  mutate(SexDE = case_when(avg_log2FC > 0 ~ 'Female',
                                           avg_log2FC < 0 ~ 'Male'),
    ...
    mutate(across(SexDE, ~factor(replace_na(.,'Not DE'), levels = c('Female', 'Male', 'Not DE'))),
           across(Bgam_sex_stocha_VKCK_bf, ~replace_na(.,'No bias')),
           across(Asex_gam_stocha_VKCK, ~replace_na(.,'No bias')),
           across(Bgam_sex_stocha_VKCK, ~replace_na(.,'No bias'))) %>%
    column_to_rownames('ID') %>%
dplyr::select(Bgam_sex_stocha_VKCK, SexDE, Bgam_sex_stocha_VKCK_bf, Asex_gam_stocha_VKCK)%>%
    rename('Sex correlation' = Bgam_sex_stocha_VKCK, 'Early sex correlation'=Bgam_sex_stocha_VKCK_bf, 'Asexual/Sexual correlation'=Asex_gam_stocha_VKCK)
```

```{r}
##ONLY ALL_DAYS
gam_asex_annot_pk <- stocha_pk_gns %>%
    filter(if_any(matches('sex_PK|gam_PK'), ~ !is.na(.))) %>%
    left_join(., lfVSlm %>% 
                  rownames_to_column("gene_id")  %>%
                  mutate(SexDE = case_when(avg_log2FC > 0 ~ 'Female',
                                           avg_log2FC < 0 ~ 'Male'),
    ...
    mutate(across(SexDE, ~factor(replace_na(.,'Not DE'), levels = c('Female', 'Male', 'Not DE'))),
           across(Bgam_sex_PK_bf, ~replace_na(.,'No bias')),
           across(Asex_gam_PK, ~replace_na(.,'No bias')),
           across(Bgam_sex_PK, ~replace_na(.,'No bias'))) %>%
    column_to_rownames('ID') %>%
dplyr::select(Bgam_sex_PK, SexDE, Bgam_sex_PK_bf, Asex_gam_PK)%>%
    rename('Sex correlation' = Bgam_sex_PK, 'Early sex correlation'=Bgam_sex_PK_bf, 'Asexual/Sexual correlation'=Asex_gam_PK)
```
...
hmap_flex = function(xprsn_mtx, cell_bcodes_annot, corr_stage1, corr_stage2, corr_stage3, key_genes=stocha_pk_gns$ID, bf1_stage, bf2_sex_to_xclud, show_gns=T, col_labs= paste0(rep(str_remove_all(asx_gams_cat_levels[str_detect(asx_gams_cat_levels, 'Gam')],'Gametocytes \\(|\\)'), each=2), c(" us", " s"))) {
  matx <- xprsn_mtx[rownames(xprsn_mtx) %in% (stocha_pk_gns %>% 
                                  filter(if_any(matches(paste0(paste(corr_stage1, corr_stage2, corr_stage3, sep='$|'),'$')), ~ !is.na(.)))%>% 
                                  filter(ID %in% key_genes)%>%
                                  pull(ID)),
                    cell_bcodes_annot %>% 
                      filter(rownames(.) %in% colnames(xprsn_mtx)) %>%
                      filter(str_detect(Stage, bf1_stage)) %>%
                      filter(!str_detect(Stage, bf2_sex_to_xclud)) %>% 
                      rownames()] %>% 
    # select(where(~! any(. >7))) %>%
    as.matrix()

  column_ha = HeatmapAnnotation(df = cell_bcodes_annot[rownames(cell_bcodes_annot) %in% colnames(matx),]  %>% 
                                  dplyr::select(Stage), #%>% 
                                  # mutate(across(Stage, ~str_remove_all(.,'Gametocytes \\(|\\)'))),
                                col = list('Stage' = cell_bcodes_annot[rownames(cell_bcodes_annot) %in% colnames(matx),] %>% 
                                             # mutate(across(Stage, ~str_remove_all(.,'Gametocytes \\(|\\)'))) %>%
                                             dplyr::count(Stage, phmap_colors) %>% 
                                             mutate_if(is.factor, as.character) %>%
                                             deframe)
                                )
  
  row_ha = rowAnnotation(df = stocha_pk_gns %>%
                           left_join(data.frame(ID=rownames(matx)), ., by='ID') %>%
                            left_join(., late_sex_de,
                                      by=c('ID'='gene_id')
                            ) %>% 
                            mutate(across(SexDE, ~factor(replace_na(.,'Not DE'), levels = c('Female', 'Male', 'Not DE'))),
                                   across(matches(paste0(paste(corr_stage1, corr_stage2, corr_stage3, sep='$|'),'$')), ~replace_na(.,'No bias'))) %>%
                            column_to_rownames('ID') %>%
                            dplyr::select(matches(paste0(paste(corr_stage1, corr_stage2, corr_stage3, sep='$|'),'$'))) %>%
                            rename('Sex correlation' = {{corr_stage1}}, 'Early sex correlation'={{corr_stage2}}, 'Asexual/Sexual correlation'={{corr_stage3}}),
                         col = list(`Sex correlation` = dr_sex_col3,
                                    `Early sex correlation` = dr_sex_col3,
                                    SexDE = de_sex_col,
                                    `Asexual/Sexual correlation` = dr_stage_col2)
                         )
  ...
  matx %>%
    Heatmap(.,
           cluster_columns = F, 
           show_column_names = F,
           row_labels = stocha_pk_gns %>%
                           left_join(data.frame(ID=rownames(matx)), ., by='ID') %>%  pull(Gene),
          show_row_names = {{show_gns}},
          left_annotation = row_ha,
          top_annotation = column_ha,
          column_split =  cell_bcodes_annot[rownames(cell_bcodes_annot) %in% colnames(matx),]  %>% 
            dplyr::select(Stage) %>% 
            mutate(across(Stage, ~str_remove_all(.,'Gametocytes \\(|\\)')),
                   across(Stage, ~factor(., levels = col_labs))),
          use_raster = F,
          column_title_rot = 90)
}
# options(repr.plot.width=7, repr.plot.height=7)
...
```{r, fig.width= 12, fig.height=10}
hmap_flex(xprsn_mtx=sorted_asx_gam_scale_data, 
          cell_bcodes_annot=sorted_asx_gam_scale_data_ffirst_bcodes,  
          corr_stage1='Bgam_sex_stocha_VKCK', 
          corr_stage2='Bgam_sex_stocha_VKCK_bf', 
          corr_stage3='Asex_gam_stocha_VKCK', 
          bf1_stage = '', 
          bf2_sex_to_xclud='late ring|early trophozoite|late trophozoite|early schizont',
          show_gns=F, 
          col_labs= paste0(str_remove_all(asx_gams_cat_levels,'Gametocytes \\(|\\)'))
          )
...
hmap_flex(xprsn_mtx=sorted_asx_gam_scale_data,
          cell_bcodes_annot=sorted_asx_gam_scale_data_ffirst_bcodes,
          corr_stage1='Bgam_sex_PK',
          corr_stage2='Bgam_sex_PK_bf',
          corr_stage3='Asex_gam_PK',
          bf1_stage = '',
          bf2_sex_to_xclud='late ring|early trophozoite|late trophozoite|early schizont',
          show_gns=F,
          col_labs= paste0(str_remove_all(asx_gams_cat_levels,'Gametocytes \\(|\\)'))
...
hmap_flex(xprsn_mtx=sorted_asx_gam_scale_data, 
          ...
          corr_stage3='Asex_gam_stocha_VKCK', 
          bf1_stage = '',
          key_genes = stocha_pk_gns %>% filter(!is.na(Bgam_sex_stocha_VKCK_bf) ) %>% pull(ID),
          bf2_sex_to_xclud='late ring|early trophozoite|late trophozoite|early schizont',
          show_gns=F, 
          col_labs= paste0(str_remove_all(asx_gams_cat_levels,'Gametocytes \\(|\\)'))
          )
...
hmap_flex(xprsn_mtx=sorted_asx_gam_scale_data, 
          ...
          corr_stage3='Asex_gam_stocha_VKCK', 
          bf1_stage = '',
          key_genes = stocha_pk_gns %>% filter(!is.na(Bgam_sex_PK_bf) ) %>% pull(ID),
          bf2_sex_to_xclud='late ring|early trophozoite|late trophozoite|early schizont',
          show_gns=F, 
          col_labs= paste0(str_remove_all(asx_gams_cat_levels,'Gametocytes \\(|\\)'))
          )
...
```{r}
scv_bfs_stochastic <- list.files("data/processed/PfDB52e/all_days/", pattern = "scv_..._stocha.h5ad", full.names = T, recursive = T, include.dirs = T)%>%
set_names(nm = (str_remove_all(.,'data/processed/PfDB52e/all_days/|/scv_|_stocha.h5ad')))%>%
map(scv$read) 
# ef_scv_stocha_all <-scv$read("data/processed/PfDB52e/all/scv_bf1_b4CR_stocha.h5ad")
```
...
```{r}
vcyto_s_us <-scv$read("data/processed/PfDB52e/all/vcyto_s_us.h5ad")
```

```{r}
#gene and cell names
bf1_bf2_genes <- vcyto_s_us$var %>% rownames()

bf1_bf2_cells <- vcyto_s_us$obs_names$values
```

```{r}
#get spliced and unspliced for all qcd genes
bf1_bf2_s <- vcyto_s_us$layers[['spliced']]$toarray() %>%
    `colnames<-` (bf1_bf2_genes) %>%
    `rownames<-` (bf1_bf2_cells) %>%
  t()

bf1_bf2_us <- vcyto_s_us$layers[['unspliced']]$toarray() %>%
    `colnames<-` (bf1_bf2_genes) %>%
    `rownames<-` (bf1_bf2_cells)%>%
  t()
```
...
```{r}
us <- CreateSeuratObject(counts = bf1_bf2_us, min.cells = 5, min.genes = 10, project = "unspliced")
### With CLR normalization
VariableFeatures(us) <- rownames(us[["RNA"]])

us <- NormalizeData(us, normalization.method = 'CLR', margin = 2) %>%
  ScaleData() %>% RunPCA(reduction.name = 'apca')

ElbowPlot(us,reduction = "apca")

us <- RunUMAP(us, dims = 1:11, seed.use = 123456, reduction = "apca", n.components = 3, min.dist = 0.3, n.neighbors = 20, spread = 1, local.connectivity = 1, a =60, b = 0.9, verbose = F)
us <- FindNeighbors(object = us, reduction = "apca", dims = 1:11, verbose = FALSE)
us <- FindClusters(object = us, resolution = 0.5, verbose = FALSE)

# Add v3 metadata to isoseq metadata
us <- AddMetaData(us, Road2UMAP@meta.data %>% select(Stage, anno_300cells, seurat_clusters, pseudo_asex, pseudo_male, pseudo_female, rank, rank2, rank3))
DimPlot(us, group.by = 'Stage', dims = c(1,2))
```

```{r}
plot_ly(data = as.data.frame(us@reductions[["umap"]]@cell.embeddings),
...
plot_ly(data = as.data.frame(us@reductions[["umap"]]@cell.embeddings),
        x = ~UMAP_1, y = ~UMAP_2, z = ~UMAP_3,
        color= us@assays$RNA@counts['PF3D7-0819800',],
        # colors = met.brewer("Klimt", 5)[1:5],
...
```{r, fig.width= 12, fig.height=10}
FeaturePlot(us, features = str_replace_all(c("PF3D7_1146800", "PF3D7_0617900", "PF3D7_1362600",  "PF3D7_0904200"),'_','-'), dims = c(1,2))
```

```{r, fig.width= 12, fig.height=10}
FeaturePlot(us, features = str_replace_all(c("PF3D7_0819800", "PF3D7_1010400", "PF3D7_0718000", "PF3D7_1348800", "PF3D7_0521900", "PF3D7_0313600"),'_','-'), dims = c(1,2))
```

```{r}
sorted_asx_gam_scale_data_us <- us@assays$RNA@scale.data %>%
as.data.frame() %>%
rownames_to_column('barcode') %>%
mutate(across(barcode, ~str_replace(., '-', '_'))) %>%
dplyr::select(barcode, any_of(sorted_asx_gam_scale_data_ffirst_bcodes %>% rownames())) %>%
column_to_rownames('barcode') %>%
as.matrix()
```

```{r, fig.width= 12, fig.height=10}
hmap_flex(xprsn_mtx=sorted_asx_gam_scale_data_us, 
          cell_bcodes_annot=sorted_asx_gam_scale_data_ffirst_bcodes,  
          corr_stage1='Bgam_sex_stocha_VKCK', 
          corr_stage2='Bgam_sex_stocha_VKCK_bf', 
          corr_stage3='Asex_gam_stocha_VKCK', 
...
```{r}
sorted_asx_gam_scale_data_ffirst_bcodes_us_s <- rbind(
  sorted_asx_gam_scale_data_ffirst_bcodes %>% 
    rownames_to_column('bcode') %>% 
    mutate(across(stage_bfs,~paste0(.,' us')),
           across(bcode,~paste0(.,'_us'))),
  sorted_asx_gam_scale_data_ffirst_bcodes %>%
    rownames_to_column('bcode') %>% 
    mutate(across(stage_bfs,~paste0(.,' s')),
           across(bcode,~paste0(.,'_s')))
            ) %>%
     column_to_rownames('bcode') %>%
     dplyr::select(stage_bfs, phmap_colors, rank2, rank) %>%
     mutate(across(c(rank2, rank), ~as.numeric(.)),
            across(stage_bfs, ~factor(.,levels = c(paste0(rep(asx_gams_cat_levels, each=2), c(" us", " s"))), ordered = TRUE))) %>%
  arrange(stage_bfs,rank2, rank) %>%
  rename('Stage' = stage_bfs)
...
```{r}
sorted_asx_gam_scale_data_us_s <- sorted_asx_gam_scale_data_us %>% 
  as.data.frame() %>% 
  rownames_to_column('ID') %>% 
  left_join(., sorted_asx_gam_scale_data %>% 
              as.data.frame() %>% 
              select(colnames(sorted_asx_gam_scale_data_us)) %>% 
              rownames_to_column('ID') , 
            by='ID', 
            suffix = c('_us', '_s')) %>%
  column_to_rownames('ID')

sorted_asx_gam_scale_data_us_s1 <- sorted_asx_gam_scale_data_us_s %>% select(any_of(rownames(sorted_asx_gam_scale_data_ffirst_bcodes_us_s)))
```
...
```{r, fig.width= 12, fig.height=10}
hmap_flex(xprsn_mtx=sorted_asx_gam_scale_data_us_s1, 
          cell_bcodes_annot=sorted_asx_gam_scale_data_ffirst_bcodes_us_s,  
          corr_stage1='Bgam_sex_stocha_VKCK', 
          corr_stage2='Bgam_sex_stocha_VKCK_bf', 
          corr_stage3='Asex_gam_stocha_VKCK', 
          key_genes=sunil_DEU$`ID DEU`, 
          bf1_stage = 'Gametocyte', 
          bf2_sex_to_xclud=' female\\)')
...
hmap_flex(xprsn_mtx=sorted_asx_gam_scale_data_us_s1, 
          ...
          corr_stage2='Bgam_sex_stocha_VKCK_bf', 
          corr_stage3='Asex_gam_stocha_VKCK', 
          key_genes=sorted_asx_gam_scale_data_us_s1 %>% rownames(), 
          bf1_stage = 'Gametocyte', 
          bf2_sex_to_xclud=' female\\)',
          show_gns = 'F')
...
```{r, fig.width= 12, fig.height=10}
# options(repr.plot.width=12, repr.plot.height=5)
sorted_asx_gam_scale_data_us_s1[rownames(sorted_asx_gam_scale_data_us_s1) %in% (stocha_pk_gns %>% 
                                  filter(!is.na(Bgam_sex_stocha_VKCK_bf)) %>%  
                                  pull(ID)), sorted_asx_gam_scale_data_ffirst_bcodes_us_s %>% filter(str_detect(Stage, 'Gametocytes')) %>% filter(!str_detect(Stage, ' female')) %>% rownames()] %>%
pheatmap::pheatmap(., 
         cluster_cols = F, 
         show_rownames = F,
         # labels_row = PfPDB_genes%>% filter(gene_id %in% (stocha_pk_gns %>% filter(!is.na(Bgam_sex_stocha_VKCK))  %>% pull(ID)))  %>% pull(Gene),
        show_colnames = F,
         annotation_row = CDgam_annot_bf,
        annotation_col = sorted_asx_gam_scale_data_ffirst_bcodes_us_s  %>%dplyr::select(Stage),
         annotation_colors = list(Stage = sorted_asx_gam_scale_data_ffirst_bcodes_us_s %>% 
                                  dplyr::count(Stage, phmap_colors) %>% mutate_if(is.factor, as.character) %>%
                                  deframe, 
                                  `Sex correlation` = dr_sex_col3,
                                  `Early sex correlation` = dr_sex_col3, 
                                  SexDE = de_sex_col),
...
gams_bf2_sce<- slingshot(gams_bf2_sce, clusterLabels = "Stage", reducedDim = 'UMAP', start.clus = c('gametocyte (developing)'),
                         end.clus = c('gametocyte (male)', 'gametocyte (female)'), shrink =1, stretch = 0, allow.break = F
)
...
```{r}
plotGeneCount(curve = asx_gams_bf1_sds, counts = counts(asx_gams_bf1_ts_sce), clusters = apply(slingClusterLabels(asx_gams_bf1_sds), 1, which.max), models = gams_bf2_ts_sce)
```

```{r}
BF1Res <- earlyDETest(asx_gams_bf1_ts_sce, knots = c(2, 1), l2fc=log2(2))
oBF1 <- order(BF1Res$waldStat, decreasing = TRUE)
head(rownames(BF1Res)[oBF1])
```


```{r}
top500_BF1 <- BF1Res %>% arrange(desc(waldStat)) %>%slice_max(order_by = waldStat,n=500) 
```
...
```{r}
top500_BF2 <- BF2Res %>% arrange(desc(waldStat)) %>%slice_max(order_by = waldStat,n=500) 
```


```{r}
plotSmoothers(gams_bf2_ts_sce, counts(gams_bf2_sce), gene = rownames(top500_BF2)[1])
```

```{r}
ggvenn(
  list('Tradeseq\nearly' = BF2Res %>% arrange(desc(waldStat)) %>%slice_max(order_by = waldStat,n=500)  %>% rownames() %>%str_replace(.,'-','_'),
    'RNA Velocity' = stocha_pk_gns %>% filter(!is.na(Bgam_sex_stocha_VKCK_bf))%>% pull(ID) ,
    'RNA Velocity\nEarly' = stocha_pk_gns %>% filter(!is.na(Bgam_sex_stocha_VKCK_bf))%>% pull(ID) #,
    # 'Pseudotime\nEarly' = stocha_pk_gns %>% filter(!is.na(Bgam_sex_PK_bf)) %>% pull(ID)
    ),
  fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#A46868"),
...
```{r}
ggvenn(
  list('Tradeseq\nearly' = BF2Res %>% arrange(desc(waldStat)) %>%
         slice_max(order_by = waldStat,n=500) %>% 
         rownames_to_column('ID') %>%
         mutate(across(ID, ~str_replace(.,'-','_'))) %>% 
         filter(ID %in% (stocha_pk_corr_genes[[16]] %>% pull(ID))) %>%
         pull(ID),
    'RNA Velocity' = stocha_pk_gns %>% filter(!is.na(Bgam_sex_stocha_VKCK_bf))%>% pull(ID) ,
    'RNA Velocity\nEarly' = stocha_pk_gns %>% filter(!is.na(Bgam_sex_stocha_VKCK_bf))%>% pull(ID) #,
    # 'Pseudotime\nEarly' = stocha_pk_gns %>% filter(!is.na(Bgam_sex_PK_bf)) %>% pull(ID)
...
stocha_pk_gns  %>% 
  filter(Bgam_sex_stocha_VKCK == 'Female'  & Bgam_sex_stocha_VKCK_anti =='Male')  %>%
  pull(ID) %>% head(.,n=10) %>% str_replace(.,'_','-')%>% 
map(. %>% plotSmoothers(gams_bf2_ts_sce, counts(gams_bf2_sce), gene = .) ) %>% 
plot_grid(plotlist = .,labels = stocha_pk_gns  %>% 
    filter(Bgam_sex_stocha_VKCK == 'Female'  & Bgam_sex_stocha_VKCK_anti =='Male') %>%
    pull(Gene)%>% head(.,n=10), ncol = 5)
```


```{r}
female_up_male_down <- stocha_pk_gns  %>% 
  filter(Bgam_sex_PK_bf == 'Female'  & Bgam_sex_PK_anti =='Male')  %>%
  pull(ID) %>% str_replace(.,'_','-')
```

```{r}
male_up_female_down <- stocha_pk_gns  %>% 
  filter(Bgam_sex_stocha_VKCK_bf == 'Male' & Bgam_sex_PK_bf =='Male' & Bgam_sex_stocha_VKCK_anti_bf =='Female' & Bgam_sex_PK_anti_bf == 'Female')  %>%
  pull(ID)%>% str_replace(.,'_','-')
```
...
```{r, fig.width=30, fig.height=15}
tseq_smoothers_all <- function(vk = 'Female', pk = 'Female', vk_anti = 'Male', pk_anti='Male'){
  gns<- stocha_pk_gns  %>% 
    filter(Bgam_sex_stocha_VKCK == vk & Bgam_sex_PK == pk & Bgam_sex_stocha_VKCK_anti ==vk_anti & Bgam_sex_PK_anti == pk_anti)  %>%
    pull(ID)%>% head(n=10) %>% str_replace(.,'_','-') 
  
  gns %>% map(. %>% plotSmoothers(gams_bf2_ts_sce, counts(gams_bf2_sce), gene = .) ) %>% 
  plot_grid(plotlist = ., labels = gns, ncol = 5) 
}
```

```{r, fig.width=30, fig.height=15}
tseq_smoothers_bf <- function(vk = 'Female', pk = 'Male', n_col=5){
  gns<- stocha_pk_gns  %>% 
    filter(Bgam_sex_stocha_VKCK_bf == vk  & Bgam_sex_stocha_VKCK_anti_bf == pk)  %>%
    pull(ID)%>% head(n=10) %>% str_replace(.,'_','-') 
  
  gns %>% map(. %>% plotSmoothers(gams_bf2_ts_sce, counts(gams_bf2_sce), gene = .) ) %>% 
  plot_grid(plotlist = ., labels = gns, ncol = n_col) 
}
```

```{r, fig.width=30, fig.height=15}
tseq_smoothers_bf(n_col=4) 
```
...
##BF1&2 combi VS BF2
##regex https://stackoverflow.com/questions/23518325/how-to-extract-substring-between-patterns-and-in-r 
smoother_flex <- function(tbl1, tbl2, tbl3, tbl4, sex1, sex2, sex3, sex4, corr_stage1, corr_stage2, corr_stage3, corr_stage4,n_col=5){
  
  gns<- stocha_pk_gns  %>% 
    filter(if_any(matches(corr_stage1), ~.== sex1) & if_any(matches(corr_stage2), ~.== sex2) & if_any(matches(corr_stage3), ~.== sex3) & if_any(matches(corr_stage4), ~.== sex4))  %>%
    pull(ID)%>% head(n=10) %>% str_replace(.,'_','-')
  
  gn_names <-stocha_pk_gns  %>% 
    filter(if_any(matches(corr_stage1), ~.== sex1) & if_any(matches(corr_stage2), ~.== sex2) & if_any(matches(corr_stage3), ~.== sex3) & if_any(matches(corr_stage4), ~.== sex4))  %>%
    pull(Gene)%>% head(n=10) %>% str_replace(.,'_','-')
  
  gns %>% map(. %>% plotSmoothers(gams_bf2_ts_sce, counts(gams_bf2_sce), gene = .) ) %>% 
  plot_grid(plotlist = ., labels = gn_names, ncol = n_col)
}
```
...
smoother_flex <- function(tbl1, tbl2, tbl3, tbl4, sex1, sex2, sex3, sex4, corr_stage1, corr_stage2, corr_stage3, corr_stage4,n_col=5){
  ...
  gns<- stocha_pk_gns  %>% 
    filter(corr_stage1== sex1 | corr_stage2== sex2 | corr_stage3== sex3 | corr_stage4== sex4)  %>%
    pull(ID)%>% head(n=1) %>% str_replace(.,'_','-')
  
  gn_names <-stocha_pk_gns  %>% 
    filter(corr_stage1== sex1 | corr_stage2== sex2 | corr_stage3== sex3 | corr_stage4== sex4)  %>%
    pull(Gene)%>% head(n=1) %>% str_replace(.,'_','-')
...
```{r, fig.width=30, fig.height=15}
  ##PK vs stocha  anti-corr cells correlation only
  print(smoother_flex(tbl1 = stocha_pk_gns, 
                 tbl2 = stocha_pk_gns, 
                 tbl3 = stocha_pk_gns, 
                 tbl4 = stocha_pk_gns, 
                 sex1 = 'Male', 
                 ...
                 sex4 = 'Female', 
                 corr_stage1 = 'Bgam_sex_stocha_VKCK', 
                 corr_stage2 = 'Bgam_sex_stocha_VKCK', 
                 corr_stage3 = 'Bgam_sex_stocha_VKCK_anti', 
                 corr_stage4 = 'Bgam_sex_stocha_VKCK_anti'))
```

```{r, fig.width=30, fig.height=15}
female_up_male_down %>% 
map(. %>% plotSmoothers(gams_bf2_ts_sce, counts(gams_bf2_sce), gene = .) ) %>% 
plot_grid(plotlist = ., labels = female_up_male_down, ncol = 2) 
```

```{r, fig.width=30, fig.height=15}
stocha_pk_gns  %>% 
  filter(Asex_gam_stocha_VKCK_bf == 'Gametocyte'  & Asex_gam_stocha_VKCK_anti_bf =='Asexual')  %>%
  pull(ID) %>% head(.,n=10) %>% str_replace(.,'_','-')%>% 
map(. %>% plotSmoothers(asx_gams_bf1_ts_sce, counts(asx_gams_bf1_sce), gene = .) ) %>% 
plot_grid(plotlist = .,labels = stocha_pk_gns  %>% 
    filter(Asex_gam_stocha_VKCK_bf == 'Gametocyte'  & Asex_gam_stocha_VKCK_anti_bf =='Asexual')  %>%
    pull(Gene)%>% head(.,n=10), ncol = 5)
```
...
library(clusterExperiment)
nPointsClus <- 20
clusPat <- clusterExpressionPatterns(asx_gams_bf1_ts_sce, nPoints = nPointsClus,
                                     genes = stocha_pk_gns  %>% 
  filter(Asex_gam_stocha_VKCK == 'Asexual')  %>%
  pull(ID) %>% str_replace(.,'_','-'))
clusterLabels <- primaryCluster(clusPat$rsec)
```
...
clusPat <- clusterExpressionPatterns(gams_bf2_ts_sce, nPoints = nPointsClus,
                                     genes = stocha_pk_gns  %>% 
  filter(!is.na(Bgam_sex_stocha_VKCK_bf))  %>%
  pull(ID) %>% str_replace(.,'_','-'), minSizes =5)
clusterLabels <- primaryCluster(clusPat$rsec)
```
...
plot_grid(plotlist = plts,  ncol = 5)
```
...
```{r}
list(Road2UMAP_list,v3_days[1:5]) %>% 
  pmap(~..1 %>% 
         colnames() %>% 
         str_remove_all('.*_') %>% 
         write(.,paste0('data/processed/PfDB52e/', ..2, '/qcd_barcodes.txt'))
       )
```
...
for (i in 1:length(v3_days[1:5])) {
  system(
    paste0('rsync -av /Volumes/GoogleDrive/My\\ Drive/PHD_documents/malaria_phd/sc_velo/data/processed/PfDB52e/', v3_days[i], '/qcd_barcodes.txt jr35@farm5-head1:/lustre/scratch118/malaria/team222/jr35/phd/V3_4TP/sc_velo/data/processed/PfDB52e/', v3_days[i])
  )
  }
